...
/** Session 272：弘农郡第六模板真实浏览器入口与渲染验收。 */
...
const hit = await evaluate(`
  const el = document.querySelector('[data-testid="btn-enter-hongnong-battlefield"]');
  if (!el) return null;
...
`);
if (!hit || hit.testId !== 'btn-enter-hongnong-battlefield') throw new Error(`弘农郡入口未命中：${JSON.stringify(hit)}`);
...
const result = await evaluate(`
  ...
  return {
    ...
    labelVisible: text.includes('弘农郡'),
    hongnongVisible: text.includes('弘农'),
    xinanVisible: text.includes('新安'),
  };
`);
if (result.templateId !== 'hongnong-190' || result.nodeCount !== 9 || result.routeCount !== 17 || result.entryCount !== 3 || result.seat !== 'hongnong_hongnong') {
  throw new Error(`弘农郡运行态异常：${JSON.stringify(result)}`);
}
if (!result.labelVisible || !result.hongnongVisible || !result.xinanVisible) throw new Error(`弘农郡渲染缺项：${JSON.stringify(result)}`);
