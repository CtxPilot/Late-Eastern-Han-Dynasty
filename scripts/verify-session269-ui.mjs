...
/** Session 269：陈留郡第三模板真实浏览器入口与渲染验收。 */
...
const hit = await evaluate(`
  const el = document.querySelector('[data-testid="btn-enter-chenliu-battlefield"]');
  if (!el) return null;
...
`);
if (!hit || hit.testId !== 'btn-enter-chenliu-battlefield') throw new Error(`陈留入口未命中：${JSON.stringify(hit)}`);
...
const result = await evaluate(`
  ...
  return {
    ...
    labelVisible: text.includes('陈留郡'),
    chenliuVisible: text.includes('陈留'),
    suanzaoVisible: text.includes('酸枣'),
  };
`);
if (result.templateId !== 'chenliu-190' || result.nodeCount !== 17 || result.routeCount !== 19 || result.entryCount !== 3 || result.seat !== 'chenliu_chenliu') {
  throw new Error(`陈留运行态异常：${JSON.stringify(result)}`);
}
if (!result.labelVisible || !result.chenliuVisible || !result.suanzaoVisible) throw new Error(`陈留渲染缺项：${JSON.stringify(result)}`);
