...
/** Session 271：河内郡第五模板真实浏览器入口与渲染验收。 */
...
const hit = await evaluate(`
  const el = document.querySelector('[data-testid="btn-enter-henei-battlefield"]');
  if (!el) return null;
...
`);
if (!hit || hit.testId !== 'btn-enter-henei-battlefield') throw new Error(`河内郡入口未命中：${JSON.stringify(hit)}`);
...
const result = await evaluate(`
  ...
  return {
    ...
    labelVisible: text.includes('河内郡'),
    huaiVisible: text.includes('怀'),
    heyangVisible: text.includes('河阳'),
  };
`);
if (result.templateId !== 'henei-190' || result.nodeCount !== 18 || result.routeCount !== 35 || result.entryCount !== 3 || result.seat !== 'henei_huai') {
  throw new Error(`河内郡运行态异常：${JSON.stringify(result)}`);
}
if (!result.labelVisible || !result.huaiVisible || !result.heyangVisible) throw new Error(`河内郡渲染缺项：${JSON.stringify(result)}`);
