...
| S16 | 存档剧本 | **M/D** | 两剧本目录、选择、白名单已可玩；v1 信封、完整 GameState/跨引用、迁移、受锁内存恢复与可序列化 PRNG 已完成；S10、S03、S07/S17 结算已收口；其余玩法 RNG、生产存取与 SQLite 仍D |
...
- **当前优先（Session 153）**：S10、S03、S07/S17、S11 结算随机流已收口；下一独立模块为 S18 家族，之后 S15 AI（另行评估 AI 决策可复现边界）。
...
*v7.3 | Session 153（S11 人事确定性结算收口）*
