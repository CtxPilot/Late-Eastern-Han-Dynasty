...
| S20 | **前端体验** | **S/D** | **Session 122 已实装：**己方武将名册、OfficerDetail、低忠诚警报、人事搜索/登用/任命/解职/赏赐统一终审窗。**Session 179 已实装：**OfficerDetail 四页签重构（属性/家族/装备/列传）+ 人物列传架构（4原型试点：biography 字段+merge脚本+4篇传记）+ 术语统一（理想替代志向）+ SVG头像辨识度优化 + 体力拆解展示（基80+加成）。**仍设计中：**W1 endTurn WebSocket+overlay；W2 数字跳动+EventLog；W3 势力凸包/Fog/tween/PCG；W4 派系面板、外交雷达、财政飘字、行政总署完整重组。0-B 前置技术债 D-0B-1~5、D-0B-9~10 |
