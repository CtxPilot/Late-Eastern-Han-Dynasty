...
| S02 | 地图 | **M/D+** | 已实装行政大地图官道邻接；**BF-P0 已完成：**南郡 190 年严格 Schema、史料校勘、16 个战场县节点/11 路线/10 地标与边界入口；**双层数据模型（Q11 已落地）：**`BattlefieldMap`（Tier I 大地图层，数字 cityId）与 `BattlefieldInstance`（Tier II 郡域场景层，字符串 countyId）保持独立不合并；**待 BF-P2～P6：**Q9 县级攻打、Q11 类型归并文档化已完成、Q12 AI 攻县依赖声明已完成；扩展至 105 郡国模板 |
...
| S16 | 存档剧本 | **M/D+** | 两剧本目录、v1 信封、完整 GameState/跨引用、迁移、受锁内存恢复与可序列化 PRNG 已完成；**BF-P2 Q10 已实装：**`activeBattlefieldInstance` 无损追加至 GameState（optional 字段，不升 schema 版本，沿用 PRNG 信封 v1 经验），互斥护栏（Zod superRefine + orchestrator 双重），`verify-save-battlefield-instance` 27/27；**待 BF-P3：**冻结战场模板版本迁移、Encounter 序列化、战场 AI 整场复现；生产存取与 SQLite 仍D |
