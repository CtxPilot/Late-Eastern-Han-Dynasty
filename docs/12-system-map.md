...
| S10 | 战斗最简 | **M+引擎/入口有缺陷** | **已实装：**六角+克制+火计+战法+暴击/反击/连击；完整单挑引擎冒烟通过，但接受挑战 API 有嵌套锁错误；战役层 Army 编成/行军/补给/围城/自动战斗可玩。**已设计：**阵型双轴成长体系（§4, Session 120） + 工程器械6种等级体系+城防体系+瓮城阶段式守城（§15, Session 121）。**仅设计：**部曲、军屯/民屯、爵位编成加成、战役层完整单挑复用与 S21 串联 |
