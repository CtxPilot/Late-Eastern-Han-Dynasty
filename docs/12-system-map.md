...
| S16 | 存档剧本 | **M+ / D+** | 两剧本目录、v1 信封、完整 GameState/跨引用、迁移、受锁内存恢复、可序列化 PRNG、浏览器 JSON 导入/导出及 XDG 命名槽位/列表/原子写入已完成；顶部系统菜单槽位 UI 已接通保存/读取确认与场景栈恢复，服务端槽位读取复用完整 Schema 与 RNG 校验；**待后续：**SQLite、多用户与云同步 |
...
| S20 | **前端体验** | **M+** | **CMD-P0～P38：**现有运行时域命令坞迁移完成；家族旧壳/DOM归零并完成共享结算总验收；Session 268 修复抽屉覆盖命令坞的交互回归并删除右栏旧内政/军事只读壳，九领域浏览器命中验证通过；屯田仍设计中 |
...
| S24 | **关系网** | **D** | 武将社交图谱 + 亲和度引擎。04§4 出身与关系网系统实装：pairAffinity 公式、六等关系状态、动态演变（同城/出征/被俘触发点）。OfficerDetail 新增「关系」tab（家族+社交融合），含婚姻（正妻+妾+姬）、血缘、社交关系列表与 SVG 径向图谱。`relations.json` 首批 20~40 对重点关系（史源分层）。妾/姬模型：`Officer.consortIds` 女性实体引用，分两档，纳妾玩法后置。详见 `docs/30-skill-tree-design.md` §S24 关联。 |
| S25 | **技能树** | **D** | 技能树化 + 点数自由加点。通用技能从平级列表改为树状结构（前置依赖），点数绑定 merit 等级。特性（被动天赋）点数化但不树化。OfficerDetail 新增「技能」tab（树状图 SVG + 特性面板）。详见 `30-skill-tree-design.md`。 |
| S26 | **天命人心** | **D** | 天命-人心双轨系统。天命值（势力宏观运势）：统一州郡+5/郡、历史功绩+10、政治阶段加成、控制天子+20。人心值（微观人际关系聚合）：武将忠诚均值×0.5 + 城池民心均值×0.3 + 关系网状态×0.2。命令坞新增「势力」入口展示双轨进度条与效果预览。详见 `31-mandate-popular-system.md`。 |
