...
| S03 | 内政 | M | 已实装农/商/城即时开发+征兵/训练/施米，随机路径统一权威 `xorshift32-v1`（即时内政确定续玩 9/9）；**民屯田 Session 339 最小切片**（分配+月结产粮+季限一次）；**军屯田 Session 345**；**Session 351 接入家属处置后的城民心/叛乱倍率**；**Session 362 文化持续投入 0-A（360金/6月/+60）**；工艺/交通/卫生仍后置；**委任AI复用本引擎（§39，设计完成）** |
> **S03 Session 362**：`culture` 复用 `activeDevelopment` 的单项目、人员门禁、月费、暂停与进度损失语义；文化值为旧档兼容 optional 运行时字段。只落库/展示，不把技术研发或人才吸引规则提前并入 S11。
