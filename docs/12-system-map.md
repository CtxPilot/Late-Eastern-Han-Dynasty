...
| S10 | 战斗最简 | **M+（三层架构已实装）** | **已实装：**六角+克制+火计+战法+暴击/反击/连击，且六角随机链已接权威 `xorshift32-v1`；完整单挑引擎冒烟通过，单挑 RNG 下一模块；战役层 Army 与三层战斗架构保持既有边界。**仅设计：**部曲、军屯/民屯、爵位编成加成、战役层完整单挑复用、S21 串联、战略点、战场工程建造、陷阱系统 |
...
- **当前优先（Session 149）**：确定性续玩分模块收口中；第一模块 S10 六角战斗已完成，下一模块为 S10 单挑。之后依次 S03 内政 → S07/S17 计谋谍报 → S11 人事 → S18 家族 → S15 AI，全部关键域完成前不得接生产读档。
...
*v6.9 | Session 149（S10 六角战斗确定性随机流收口）*
