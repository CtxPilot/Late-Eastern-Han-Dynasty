...
| S08 | 外交 | M+ | 进贡/结盟/**献美**+金同步；R2 结盟百分点公式、权威 RNG 与 UI 同源（20/20），声望/戒备字段仍未实装 |
...
| ID | 系统 | 成熟度 | 要点 |
|:--:|------|:------:|------|
| S11 | 人事 | **M+** | **仅男将**搜索/登用已接权威 PRNG（32/32）；R2 修复义理/野心百分点并与 UI 同源（合并验证20/20）；赏赐；**任命三轨**；**君主身份特例（Session 188）：君主不参与忠诚度/拉拢记录/功绩系统，UI 隐藏相关区块，引擎守卫属切片 C 待落地，规则见 04 §3.8** |
| S12 | 官职功绩体力 | **S/M+** | **0-A 精简任命**；功绩等级字段代码未实现，表在文档；功绩门槛未联动任命；体力已完整接入（Session 63/65）；**委任军团官职/爵位门槛（§39，设计完成）**；**君主不参与功绩等级系统（Session 188，见 04 §3.8/§6.5），UI 改显示势力综合国力**
| S13 | 宝物 | S | — |
