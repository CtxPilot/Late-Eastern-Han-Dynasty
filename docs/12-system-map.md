...
| S10 | 战斗最简 | **M+（三层架构已实装）** | **已实装：**六角+克制+火计+战法+暴击/反击/连击与完整单挑均接权威 `xorshift32-v1`（5/5 + 3/3）；战役层 Army 与三层战斗架构保持既有边界。**仅设计：**部曲、军屯/民屯、爵位编成加成、战役层完整单挑复用、S21 串联、战略点、战场工程建造、陷阱系统 |
...
| S16 | 存档剧本 | **M/D** | 两剧本目录、选择、白名单已可玩；v1 信封、完整 GameState/跨引用、迁移、受锁内存恢复与可序列化 PRNG 已完成；所有行动后结算 RNG 已收口；S15 行为复现、生产存取与 SQLite 仍D |
...
*v7.9 | Session 161（S15 军事 AI 接入 CampaignArmy）*
