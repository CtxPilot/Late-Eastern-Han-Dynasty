...
| S10 | 战斗最简 | **M+（三层架构已实装）** | **已实装：**六角+克制+火计+战法+暴击/反击/连击；完整单挑引擎冒烟通过，但接受挑战 API 有嵌套锁错误；战役层 Army 编成/行军/补给/围城/自动战斗可玩；**三层战斗架构实装：**战场地图画面（Tier I 节点子集渲染/行军/交战）+ 白刃战标准模式（Tier II 战术点系统/阵型选择/runMeleeRound引擎）+ 入口三选弹窗。**仅设计：**部曲、军屯/民屯、爵位编成加成、战役层完整单挑复用、S21 串联、战略点、战场工程建造、陷阱系统 |
...
| S16 | 存档剧本 | **M/D** | 两剧本目录、选择、白名单已可玩；v1 信封、完整 GameState/跨引用、迁移、受锁内存恢复与可序列化 PRNG 已完成；三级战斗均已收口；各玩法 RNG 接入、生产存取与 SQLite 仍D |
...
- **当前优先（Session 148）**：继续 S16 Gate 2；确定性续玩已拍板，可序列化 PRNG/信封/恢复基础已完成。下一步从战斗与单挑开始，逐域把随机调用接到服务端权威随机源并补消费序列回归；全部关键域收口前不得接生产读档。之后回到势力特点数据 → 委任军团 → AI Army → 战略点。
...
*v6.8 | Session 148（S16 可序列化 PRNG 与确定性续玩基础契约）*
