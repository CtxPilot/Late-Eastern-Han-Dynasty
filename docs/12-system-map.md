...
> v4.3 起 22 大系统；v4.5 新增 S23 人物状态表情系统（挂 S22 美术基调 C 层状态化扩展）；v4.4 S22 D→S/D 部分实装。
...
| S02 | 地图 | **M/D+** | 已实装行政大地图官道邻接；**BF-P0 已完成：**南郡 190 年严格 Schema、史料校勘、16 个战场县节点/11 路线/10 地标与边界入口；**双层数据模型（Q11 已落地）：**`BattlefieldMap`（Tier I 大地图层，数字 cityId）与 `BattlefieldInstance`（Tier II 郡域场景层，字符串 countyId）保持独立不合并；**BF-P2 Q9 已完成：**首批 3 县（当阳/华容/枝江）可攻打——`engageCounty(countyId)` + `tickBattlefieldInstance` 月度 tick；**BF-P4 已完成：**颍川第二郡17县/29陆路对照+双入口单挑；**BF-P5 Seed 工具已实装：**`CommanderySeed` → `buildHistoricalGeographyBundle` 纯函数构建器+`pnpm verify-historical-geography` 逐郡校验；南郡/颍川均已迁移至 seed 生成；**BF-P5 补给线真实路径判定已实装（Session 254）：**`shared/army-county-mapping.ts` Army—县位置映射 + `tickBattlefieldInstance` 逐军补给线路径判定（经过攻方控制县 → 粮耗×2 + 士气-5），替换全局简化。**BF-P5 orchestrator 去硬编码已实装（Session 255）：**`shared/commandery-templates.ts` 郡国模板目录驱动 `enterNanjunBattlefield`/路由校验/逐郡校验/前端标签，新增郡国只需登记目录条目。**BF-P5 郡域迷雾已实装（Session 256）：**`shared/commandery-fog.ts` 纯函数（揭示集 = 入口县 ∪ 郡治 ∪ 攻方 Army 所在县 ∪ 攻方已占领县 ∪ **守方 Army 所在县（R6，Session 258 并入）**，每源 + 一跳邻接）+ `maskGameStateForPlayer` 集成；`foggedNodeIds` 为 mask 投影专属字段（Zod optional，不入存档）；地理层恒可见、军情层按揭示集遮蔽；占领县→邻接破雾=视野扩张攻占效果（BF-P2 Q9 第 4 项完整实现）。验证：fog 单测 10/10、`verify-save-battlefield-instance.ts` **74/74**（f8 迷雾 + f9 守方 Army 入郡域）、真实 API + Headless Chrome 闭环。**BF-P5 年代覆写机制已实装（Session 257）：**`seed-schema.ts` 县/地标/路径/郡定义均支持 `validFromYear/validToYear`（缺省=scenarioYear）；`resolveBundleForYear` 按年份过滤并重新 Zod 校验，无效年份或引用断裂抛错（无静默回退）；南郡/颍川仍为 190 单一年代，多年代能力由测试夹具演示。**守方 Army 入郡域场景已完成（R6 范畴，Session 258）：**`enterNanjunBattlefield` 守方势力改为郡治城市（worldCityId）实际占领势力，驻留郡治的守方现役 Army 自动纳入并部署到守方纵深前沿县；迷雾揭示源并入守方 Army 所在县（deployments 快照仍不泄露）；补给线真实路径判定在真实流程触发（f9，verify-save-battlefield-instance **74/74**）。**待 BF-P5 后续：**第三郡录入（需史料）、扩展至 105 郡国模板 |
...
| S10 | 战斗最简 | **M+（R3完成）/D+** | **已实装：**六角+克制+火计+战法+暴击/反击/连击与完整单挑均接权威 `xorshift32-v1`；R3 四倾向权威链与吕布“规则内最强但可败”已完成，败局只改写斩俘处置。现有 Tier I/II 状态壳可运行。**待 BF-P2～P4：**历史郡域深化、自动/标准/六角统一 Encounter、场景内单挑 |
...
| S20 | **前端体验** | **M+** | **CMD-P0～P38：**现有运行时域命令坞迁移完成；家族旧壳/DOM归零并完成共享结算总验收；屯田仍设计中 |
...
- 下一步建议 P35 先审计家族现有读写链；屯田没有运行时，不得伪造迁移。
