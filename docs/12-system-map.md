...
- **下一优先**：总军师系统实装（任命/态势切换/献策/对决）→ 设施建造回合化（大型器械消耗完整回合）→ 势力特点数据实装 → AI Army 接入 → 战役 UI 扩展（行军箭头/围城视觉）。**S20 已由 Session 122 开始部分实装**；可继续 W4，其余 S20/S21 工作包仍按技术储备排期。
...
*v5.5 | Session 122（S20 武将名册/详情与人事终审窗部分实装）*
