...
| S16 | 存档剧本 | **M/D** | 两剧本目录、选择、白名单已可玩；v1 信封、完整 GameState/跨引用、迁移、受锁内存恢复与可序列化 PRNG 已完成；S10 战斗/单挑与 S03 内政已收口；其余玩法 RNG、生产存取与 SQLite 仍D |
...
*v7.1 | Session 151（S03 内政确定性随机流收口）*
