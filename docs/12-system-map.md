...
| 概念 | 定义 |
|------|------|
| **宫廷人脉 S09** | 势力机会库存；赏赐/牵线/谍报掩护/宫廷交涉；旧代码字段 `beauty*` 待迁移 |
...
| S02 | 地图 | **M/D+** | 已实装行政大地图官道邻接；**BF-P0 已完成：**南郡 190 年严格 Schema、史料校勘、16 个战场县节点/11 路线/10 地标与边界入口；**双层数据模型（Q11 已落地）：**`BattlefieldMap`（Tier I 大地图层，数字 cityId）与 `BattlefieldInstance`（Tier II 郡域场景层，字符串 countyId）保持独立不合并；**BF-P2 Q9 已完成：**首批 3 县（当阳/华容/枝江）可攻打——`engageCounty(countyId)` + `tickBattlefieldInstance` 月度 tick；**待 BF-P3～P6：**动态战况、AI 整场复现、第二郡、扩展至 105 郡国模板 |
...
| S09 | **宫廷人脉（迁移中）** | M+/D | 目标语义已与女性人口脱钩；现有寻访/战后转移仍使用旧 `beauty*` 字段和规则，确定续玩 25/25；R7 待迁移 |
| S10 | 战斗最简 | **M+/D+（三层壳已实装）** | **已实装：**六角+克制+火计+战法+暴击/反击/连击与完整单挑均接权威 `xorshift32-v1`（5/5 + 3/3），现有 Tier I/II 状态壳可运行。**已批准待 BF-P1～P4：**历史郡域必经层、自动/标准/六角统一 Encounter、场景内单挑；六角保留为可选局部微操，不推倒重做 |

### B. 人 / 家族 / 阴谋

| ID | 系统 | 成熟度 | 要点 |
|:--:|------|:------:|------|
| S11 | 人事 | **M+** | **仅男将**搜索/登用已接权威 PRNG（32/32）；R2 修复义理/野心百分点并与 UI 同源（合并验证20/20）；赏赐；**任命三轨**；**君主身份特例（Session 188）：君主不参与忠诚度/拉拢记录/功绩系统，UI 隐藏相关区块，引擎守卫属切片 C 待落地，规则见 04 §3.8**；**霸府/称王/称帝主线（docs/26，HC-P0~P2 已批准）：Q2 方案B 新增 `Officer.hegemonyPosition?` 独立轨道承载霸府/王国/帝国专属官职，与三轨并存** |
| S12 | 官职功绩体力 | **S/M+** | **0-A 精简任命**；功绩等级字段代码未实现，表在文档；功绩门槛未联动任命；体力已完整接入（Session 63/65）；**委任军团官职/爵位门槛（§39，设计完成）**；**君主不参与功绩等级系统（Session 188，见 04 §3.8/§6.5），UI 改显示势力综合国力**；**霸府/称王/称帝主线（docs/26，HC-P0~P2 已批准）：`Faction.politicalStage` 政治阶段状态机 + `Faction.politicalTitle` 头衔独立字段（Q7 方案B），与 FactionTrait 势力性格分离（Q10）**
| S13 | 宝物 | S | — |
| S14 | 事件 | **M+** | 场景/史料层隔离 + 年月窗口/前置/互斥/失效 + 玩家/AI选择 + EventDialog来源标签；190共24事件/5条叙事线 |
| S15 | AI | **M+** | 内政占位 + 谍报/计谋 + **外交过滤/君主激进度/真实 CampaignArmy 出征占城/战报** + 总军师自动态势切换；军事决策与结算接权威 RNG，完整多线战略仍未做 |
...
| S18 | **家族** | **M+** | 女眷/婚配/跟随 ✅；跟随/默认忠诚接权威 PRNG（32/32）；**固定子女登场引擎** ✅；父辈/族谱 ❌ |
| S19 | **单挑大会** | **D** | 独立锦标赛系统：赛制/押注/称号/叙事/数据结构设计完成，引擎待实现 |
| S20 | **前端体验** | **S/D** | **Session 122 已实装：**己方武将名册、OfficerDetail、低忠诚警报、人事搜索/登用/任命/解职/赏赐统一终审窗。**Session 179 已实装：**OfficerDetail 四页签重构（属性/家族/装备/列传）+ 人物列传架构（4原型试点：biography 字段+merge脚本+4篇传记）+ 术语统一（理想替代志向）+ SVG头像辨识度优化 + 体力拆解展示（基80+加成）。**Session 196 CMD-P1 已实装：**§12 通用命令坞/抽屉无业务壳层与瞬时草稿 reducer，和旧手风琴并存；朝廷业务尚未接入。**仍设计中：**W1 endTurn WebSocket+overlay；W2 数字跳动+EventLog；W3 势力凸包/Fog/tween/PCG；W4 派系面板、外交雷达、财政飘字、行政总署完整重组。0-B 前置技术债 D-0B-1~5、D-0B-9~10 |
