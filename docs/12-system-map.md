...
| S16 | 存档剧本 | **M/D** | 两剧本目录、选择、白名单已可玩；v1 信封、完整 GameState/跨引用、迁移、受锁内存恢复与可序列化 PRNG 已完成；非 S15 玩家/共享结算 RNG 已收口；三个 S15 决策文件、生产存取与 SQLite 仍D |
...
*v7.7 | Session 157（RNG 确定性收口计划阶段性完成）*
