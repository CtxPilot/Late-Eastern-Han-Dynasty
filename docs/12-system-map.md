...
| S10 | 战斗最简 | **M+/战役实装** | 六角+克制+火计+**战法引擎**+三级水军(走舸/蒙冲/楼船)+**暴击/反击/连击引擎最小切片(§6.2~6.5·专属·防循环·阵型修正·已触发)**+**单挑引擎最小切片(§8全自动结算·7指令三向克制·专属·无双保护·DuelPanel·已可玩)**+**主副将与参谋编成(独立槽位·智≥85·幕僚不带兵·爵位叠加·上限9~10人)**+**部队品质(经验Lv1-7+组织度0-100+士气深化)**+**部曲系统(12史载武将)**+**军屯田+家属质任制**+**战役层引擎最小切片(§12节点·§13Army编成+行军+补给·§15设施·§16状态机·§17自动战斗算法·CampaignPanel UI·已可玩)**+**单挑演出层设计(DuelStage混合范式·Konva.Animation+layer.getContext()命令式·卡牌展示·技能特写·原生Web Audio程序化合成)**+**HeroCharacter特殊造型(appearance字段落库·Canvas 2D filter气劲/残影/光环·吕布鬼神降临Verlet雉翎+烈焰足+帧缓存残影+鬼神觉醒+登场杀)**；宿命对决/大会/战后延续/总军师态势接入/AI Army后置；水域移动后置 |
...
*v4.4 | Session 102（跨平台字体防御实装：资产闭环 @font-face + Canvas 屏障 FontBarrier + Konva fontFamily + .editorconfig/.gitattributes/CI 编码门禁。S22 D→S/D。Linux UI 适配 P5-07a~e 留档）*
