...
| `garrisonFamilies` | number? | 征兵绑定的本城驻军家属口数；每 1 兵最多绑定 2 口 |
| `familyBackupCityId` | number? | 质任后家属所在后方城 |
...
## 二十六、家属质任处置数字真源（S18/S21，Session 351）

> 本节是 `shared/hostage-families.ts` 的数值真源。改变数值时先改本节，再同步
> `docs/05-combat-system.md`、`docs/34-faction-politics-design.md` 与实现。

| 项 | 数值 | 说明 |
|----|------|------|
| 征兵绑定 | 每 1 兵最多 2 口 | `min(兵×2, adultFemale+child+elder)` |
| 迁家属 | 500 金 | 每城每季一次 |
| 家属所在城失陷 | 相关旧主驻军士气 −40 | 首都质任有全国冲击；失陷城本身不重复扣 |
| 善待民心 | 新占城 `stats.morale` +10 | 夹在 0~100 |
| 善待余波 | 相关旧主驻军每季士气 −5 | 持续 3 个后续季度，之后状态清除 |
| 善待叛乱倍率 | ×0.7 | 乡政派系月度叛乱判定 |
| 中立 | 无追加效果 | 仅保留已发生的 −40 冲击 |
| 镇压民心 | 新占城 `stats.morale` −20 | 夹在 0~100 |
| 镇压攻城 | 相关旧主攻城战力 ×1.1 | 仅当前 0-A 战役自动攻城路径 |
| 镇压叛乱倍率 | ×1.5 | 乡政派系月度叛乱判定 |

运行时字段：`GameState.pendingFamilyTreatment` 为玩家待决项；玩家选择后写入占领城
`City.familyTreatment`（旧主势力、影响城市、季度起止）。待决项会阻塞结束回合并进入 v1
存档。六角直接战斗仍不读取处置倍率，待后续统一接入战斗模式时再扩展。

## 二十七、六角协同包围与战术撤退数字真源（S10，Session 352/358/360/367）

> 本节是 `shared/hex-positioning.ts` 与六角战斗/战役结算的数值真源。它是瞬时派生规则，
> 不新增存档字段；改变阈值或回流比例时先改本节，再同步 `docs/05-combat-system.md`、
> `docs/06-api-design.md`、`docs/07-ui-design.md` 与实现。

| 项 | 数值 | 说明 |
|----|------|------|
| 成立门槛 | 至少 2 个不同接战方向 | 敌军须与目标相邻，且朝向允许接战；背向单位不计 |
| 排除条件 | 0 兵力、`isDestroyed` 或 `isRetreated` | 仅存活且仍有兵力的单位参与派生 |
| 受围变阵 | 仅方阵 | 其他阵型由服务端拒绝；不消费 TP/RNG |
| 战术撤退阻断 | 任一我军部队受围 | 返回 `RETREAT_SURROUNDED`，快照不变 |
| 撤退回流率 | 50% | `/battle/exit` 结算；存活攻方先写 `isRetreated` |
| 敌军主动撤退·士气门槛 | ≤20 | 敌军 AI 在目标选择前检查；仅未受协同包围单位适用 |
| 敌军主动撤退·兵力门槛 | ≤25% 最大兵力 | `troopCount / maxTroops ≤ 0.25`；仅未受协同包围单位适用 |
| 敌军主动撤退·截击距离 | 1 格 | 满足撤退门槛但有相邻活跃敌对单位时，本次不得直接标记 `isRetreated` |
| 截击后目标优先级 | 相邻活跃截击者 | 截击成立时先在相邻活跃敌对单位中按既有确定性评分选目标；无候选才回退普通目标评分 |
| 追击触发 | 被截击的撤退尝试 | 满足撤退门槛但被 1 格相邻活跃敌对单位截击时，触发追击 |
| 追击伤害系数 | 0.6 × 基础伤害 | 取 `calcDamage` 中位值（随机 1.0）×0.6，至少 1；不触发暴击/反击/连击，士气 −2；若归零则 `isDestroyed` |
| 追击方选择 | 最强相邻截击者 | 按既有目标评分在相邻活跃敌对单位中择优（威胁/距离/兵力/Id 稳定排序），仅一名追击者出手 |
| 玩家战术撤退追击 | 同追击系数 | `retreatBattle` 成功时，每支与活跃守军相邻的攻方退兵按同系数承受一次追击；单次/单名截击者结算 |

`乱战` 暴击链继续使用既有技能/阵型数值；本切片只把派生 `isSurrounded` 接入攻方与反击判定，
不新增另一套暴击公式。Session 354 的敌军 AI 只在已有一翼时寻找第二个可达邻接方向，不新增数字字段或 RNG 消费；
Session 355 再在受围时寻找解除派生态势的可达空格，仍不新增数字字段或 RNG 消费。Session 357 将上述两个门槛接入
敌军 AI：触发后复用 `isRetreated` 终态，若敌军已无活跃单位则判定玩家胜利；受围单位不跳过突围/接战逻辑。Session 358
补入最小相邻截击门禁：满足撤退门槛但与任一活跃敌对单位相邻时，不直接标记撤退，写入截击战报并继续既有战法/火计/普攻链，
不消费额外 RNG、不新增追击状态。Session 360 再将截击后的行动目标收紧为相邻活跃截击者优先，
避免全局目标评分把被截敌军引向远处目标；仍复用原战法/火计/普攻链与 RNG 顺序。Session 367 在不新增字段/API/RNG 消费的前提下，
为“被截击”补入一次追击伤害：敌军被截击时由最强相邻截击者结算 0.6 系数基础伤害（中位值、必中），写战报并削减被截单位兵力/士气；
玩家 `retreatBattle` 成功时对每支相邻退兵同系数追击；未被截击的撤退、全军撤退终局与 50% 回流保持不变。本轮仍不含攻城突围与多军团规则。

## 二十八、攻城守城与突围数字真源（S10，Session 368）

> 本节是六角 `BattleState.isSiege` 与城门突围的数值真源。它是瞬时派生/战斗修正规则，
> 不新增存档字段；改变数值时先改本节，再同步 `docs/05-combat-system.md`、`docs/06-api-design.md`、`docs/27-tactical-wargame-system.md` 与实现。

| 项 | 数值 | 说明 |
|----|------|------|
| 城内守军防御加成 | +30% 有效防御 | `isSiege` 时守方 `calcDamage` 的 `formationDef` 额外 +3 点（约 +30%，六角 `hexFormationMods` 外的攻城修正，不与阵型点值叠算组织度） |
| 城门突围放宽 | 边缘 1 格 | `isSiege` 且攻方 `retreatBattle` 时，位于地图边缘（`q==0`/`q==width-1`/`r==0`/`r==height-1`）的受围单位不计入 `RETREAT_SURROUNDED` 阻断；该单位可经城门突围 |
| 突围追击 | 同 08 §二十七追击系数 | 经城门突围的单位仍按 0.6 系数承受相邻守军追击（单名最强者） |
| 随机性 | 0 次 RNG | 守城加成与突围门禁均为确定性规则 |

Session 368 在不新增字段/API/RNG 的前提下，为 `isSiege` 六角战补入守城防御加成与城门突围门禁；城内/野战的其余包围、追击、回流与多军团规则仍按 §二十七执行。

## 二十九、移动后冲锋数字真源（S10，Session 371）

> 本节是六角战「骑兵移动后冲锋」的数值真源。它是普攻瞬时修正规则，不新增存档字段；
> 改变数值时先改本节，再同步 `docs/05-combat-system.md`（§2 地形表 / §4 阵型表 / §5 兵种表 / §6.4 连击）与实现。

| 项 | 数值 | 说明 |
|----|------|------|
| 冲锋触发前置 | 轻/重骑兵 + 本回合已移动 + 普攻 | 复用 `hasMovedThisTurn` 语义（与强行军连击同源）；仅玩家 `attackUnit` 与敌军 AI `doAttack` 普攻路径生效，**战法/火计/追击不触发** |
| 平原冲锋加成 | +20% 伤害 | 攻方落点地形为平原（05 §2 地形表「骑兵冲锋+20%」） |
| 重骑兵加成 | +50% 伤害 | 兵种特性（05 §5「冲锋首击+50%」；0-A 简化为每次冲锋均生效，战斗内“首击一次”状态后置 0-B） |
| 冲阵阵型加成 | +80% 伤害 | 阵型 id=16（05 §4「仅骑兵，冲锋+80%」），与上述来源独立叠加 |
| 锥形阵骑兵突击 | +50% 伤害 | 阵型 id=2 仅骑兵（05 §4「骑兵突击伤害×1.5」；`formations.json` effects `charge=50, cavalry_only` 同值），Session 377 接入冲锋乘区 |
| 加成合成 | 加法叠乘 | 总 pct = 各来源之和；`baseDamage × (1+pct/100)` 后取整；pct=0 视为未触发（如森林轻骑按普通攻击处理，无「冲锋」标签） |
| 骑神连击联动 | 连击率 +20%；冲阵时连击系数 0.6→0.72 | 仅马超（骑神）冲锋中生效（05 §6.4.3）；0.72 即冲阵连击伤害 ×1.2 |
| 战报标签 | 「冲锋」 | 写入攻击事件 labels，展示于战报〔〕段首位 |
| 随机性 | 0 次额外 RNG | 冲锋为确定性乘区，不改变暴击/反击/连击的既有 RNG 消费顺序 |

Session 371 在不新增字段/API/RNG 的前提下落地骑兵冲锋最小切片；锥形阵(2)「骑兵突击×1.5」、
猛进/先登特性与 0-B 特殊兵种冲锋数值（虎豹骑+80% 等）仍后置。


## 三十、六角战术视野数字真源（S10，Session 378）

> 本节是六角战「战术视野」（玩家侧信息投影）的数值真源。它是纯 UI 投影规则，
> 不新增存档字段、不消费 RNG；改变数值时先改本节，再同步 `docs/05-combat-system.md`
> §3.1 与实现 `shared/battle-sight.ts`。

| 项 | 数值 | 说明 |
|----|------|------|
| 单位基础视野 | 4 格 | 六角距离，全部兵种统一（0-A 简化，按兵种差异化后置） |
| 山地修正 | +1 格 | 观察者所在格为山地（登高望远） |
| 森林修正 | −1 格 | 观察者所在格为森林（遮蔽观察） |
| 雾天修正 | −2 格 | 与雾天射程 −2 同源取值 |
