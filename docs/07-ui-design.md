...
├────────────┬──────────────────────────┬────────────────────────┤
│            │                          │                        │
│  LeftPanel │  StrategicWorldView      │     RightPanel        │
│            │  （天下→州→城卡片）         │                        │
│  谍报      │                          │   选中: [新野]         │
│  ├─招募    │  [州卡片网格]             │   势力: 刘备          │
│  ├─女间谍  │  [城卡 / 官道邻接]         │   金/粮/农/商/城      │
│  ├─派遣    │  （已取消连续大地图）      │   兵力/士气/民心      │
│  └─反间    │                          │                        │
│            │                          │   [开发] [征兵] [训练] │
│  计谋      │                          │   [施米] [寻访] [出征] │
│  ├─美人计  │                          │                        │
...
左侧面板(240px / w-60)：
  - 手风琴式折叠，默认全部收起
  - 折叠项（按实际代码顺序）：战役、总军师、己方城池（快捷列表，点击定位战略卡片并选城）
  - 内政、军事、人事、外交、计略、情报、家族、朝廷均已迁入底部命令坞，左右栏旧操作入口物理删除
  - 结束回合仅在 TopBar；命令坞“进行”仍是禁用状态提示
```

### 5.2 世界屏战略卡片（Session 379 · 取代 MapCanvas）

> **Session 379**：连续疆域大地图（Konva `MapCanvas` + `geo-basemap`）退出世界屏主路径。
> 中央主区改为 `StrategicWorldView`：天下形势 → 州卡片 → 城卡片。
> 选城仍写 `selectedCityId`（RightPanel / 命令坞契约不变）。
> 旧 MapCanvas 迁入 `client/src/components/map/legacy/`，地理数据与底图资产保留，见根目录 `MAP_REMOVAL_ANALYSIS.md`。

```
交互层级：
  天下形势（十三州卡片网格）
    → 州（该州城池卡片；0-A 按 city.province 聚合）
      → 选城（selectedCityId → RightPanel / 命令坞）

州卡派生字段（无新存档）：
  主控势力 / 占城比 / 城数 / 人口粮兵合计 / 战事标记

城卡：
  治所·关隘·己方标记 / 控制势力色 / 金粮兵口 / 官道邻城名
...
Layer 2 — 部队
  ├─ 每支部队一个小旗杆 + 图标
  ├─ 旗帜颜色 = 势力
  ├─ 正式交战双方姓名读取 BattleUnit.commanderName，不反查可能被迷雾裁剪的 game.officers
  ├─ 选中部队 → 绿色六角格高亮
  └─ 移动范围 → 淡黄色六角格高亮
...
底部操作栏(高 60px)：
  ...
  [阵型 ▼]  — 下拉选择可用阵型(当前+切换消耗)
  [待机]     — 结束本队行动
  [战术撤退] — 玩家回合发起有序撤退；受协同包围时由服务端阻断
```

**六角协同包围与战术撤退（Session 352）**：BattleView 顶部的
`data-testid="battle-surround-status"` 显示当前受协同包围的我军部队数量；未形成包围时显示
“协同包围：未形成”。玩家回合显示 `data-testid="btn-battle-retreat"`，点击后提交
`POST /api/game/battle/retreat`；服务端返回 `RETREAT_SURROUNDED` 时战报将其翻译为“被协同包围，
无法撤退”，不改变快照。成功后战斗进入结束态，按钮变为 `btn-exit-battle`，败方从战场返回大地图；
撤退战报和 50% 残部回流由 `/battle/exit` 完成。该 UI 只展示服务端派生结果，不在客户端自行判定
包围合法性。

**撤退态 UI 边界（Session 361）**：`BattleUnit.isRetreated=true` 的部队仍保留在战斗快照，
但 BattleView 不再将其绘制为可选部队、攻击目标、红色可攻击标记或协同包围来源；滞留的
...
> 选择随挑战请求进入服务端，演出面板显示双方权威倾向。刷新时若存在进行中六角战斗，
> 客户端会恢复到 BattleView。应战方当前仍由 §8.13.1 自动接受/拒绝并独立选择倾向。
...
> **Session 187 更新**：体力基础值已缩放封顶 100（Session 186），与五维同刻度，并入原五维区块为**六维**，使用统一进度条样式（相同渐变/数值标注/标签风格），aside 不再有独立体力数字卡片。进度条超 100 时按 0-100 填满、超出部分数值标 `100 (+N)`；动态战斗消耗状态不在详情页展示，只显示静态基础值。
...
│  政: 63  █████████░░░░░░░      │
│  魅: 94  ██████████████░░      │
│  体:100  █████████████████      │  ← 体力并入第六项（基础值≤100，加成叠加可突破）
│                                 │
│  技能: 武圣(专属) / 疾驰Lv3     │
│        火计Lv1 / 鼓舞Lv2        │
│                                 │
│  装备: [青龙偃月刀] [—]         │   ← Session 266 起真实 5 槽（主武器/副武器/铠甲/坐骑/兵书）
│        [赤兔马]      [—]         │      装备 tab 展示宝物名/品质/属性/效果 + 卸下；势力库存 + 赏赐并装备
│                                 │
│           [关 闭]               │
└─────────────────────────────────┘
```
...
> 详见 `24-character-expression-system-design.md`。本轮仅 3 原型（吕布 id=5 / 曹操 id=1 / 诸葛亮 id=4）+ 程序化 SVG 占位。

**接入点 1 — BattleView SideCard（瞬时态验证）**：
`BattleView.tsx` 的攻/守方 SideCard（`battle.ts:233-250` 现有 `SideCard` 组件）旁加 `<ExpressionPortrait officer battle={...} compact />`。
数据源：`battle.winner` + `battle.units.find(u=>u.commanderId).side/morale/isDestroyed`。
胜仗 → `victory`/`neutral`（按原型）+ `gold` 背景；败仗 → `defeat`/`anger`/`ponder`（按原型）+ `cold` 背景。
非 3 原型武将走通用回退（仅 `neutral`/`suspicion`/`ponder`，无角色化变体）。

**接入点 2 — OfficerDetail 大头像（持续态验证）**：
`OfficerDetail.tsx:89` 的 `<OfficerPortrait officer={officer} />` 替换为 `<ExpressionPortrait officer={officer} />`（不传 battle，走持续态）。
数据源：`officer.loyalty` 区间（<60 → `suspicion`/`reluctant`）+ `officer.stamina` 区间（<30 → `ponder`/`anger`）+ `officer.hidden/stats`（原型化规则）。
可选补充士气：`campaignArmies.find(a=>a.commanderId===officer.id)?.morale`。

**不接入**：OfficerRosterPanel 名册缩略图（compact 尺寸表情难辨）、EventDialog 对话头像（`EventDialogue.portrait` 是静态字符串，需改 schema）。
...
> **实装状态**：设计完成，UI 待实现。CMD-P4 后应归入底部“朝廷”命令抽屉，不再引用已删除的
> 左栏“君主”手风琴。
...
### §11.2 S21 战争四层串联（W6~W9）

#### §11.2.1 四层架构、三种交战结算模式与状态机
...
## §12 大地图命令 UI（朝廷域最小安全闭环已实装）

> 本节只定义未来大地图的统一命令入口与交互协议，不代表对应玩法已经实装。首版落地仍须按
> `docs/12-system-map.md` 的系统成熟度逐项开放，禁止为填满菜单临时新增运行时切片。
>
> **Session 199 / CMD-P4 状态**：通用 `CommandDock`、`CommandDrawer` 与纯客户端草稿
> reducer 已挂入 world `GameLayout`；“朝廷”成为首个真实业务域，接入君主/政统、汉帝控制、
> 皇权/冷却、开府、伪诏及霸府官制只读总览。`CommandConfirmDialog` 已原位升级为统一终审，
> 补齐焦点圈定/归还、对象化唯一标题和提交前权威快照再校验。其余领域仍显示原面板/设计中去向，
> “进行”仍指向顶部既有入口；旧“君主”手风琴及其本地确认分支已原子删除，朝廷命令坞是
> 政治阶段、皇权、开府和伪诏宣战的唯一写入口。
...
- `进行（结束本月）`为独立朱砂主按钮，不并入九类命令；系统设置、存储、载入放在左上系统菜单；
...
#### 12.1.1 首批实施布局契约（Session 196 / CMD-P0 固化）
...
- 抽屉从命令坞上沿向上展开，桌面宽 `360～420px`，最大高度不超过三栏内容可用高度，内容区
  独立滚动。首批动画固定为 `180ms ease-out` 展开、`140ms ease-in` 收起，并尊重
  `prefers-reduced-motion: reduce`（降为无位移动画）；动画期间不重复响应同一触发。
- 抽屉关闭方式为：再次点击当前领域、点击显式关闭按钮或按 `Esc`。不以点击地图作为隐式关闭，
  避免地图选点与草稿丢失耦合。
...
- 壳层只管理领域、命令标识和瞬时草稿，不读取业务 `GameState`、不计算门槛、不调用 API。
- 选择命令可建立草稿；更新参数只替换对应领域草稿。终审取消回到抽屉并保留草稿；命令提交成功、
  显式关闭抽屉、切换领域时清除相关草稿。
- 服务端失败不等同于提交成功：终审及草稿均保留，直到玩家修改或关闭。
- 游戏重开、切换剧本/势力、离开 world 场景时重置整个壳层，防止跨局草稿污染。
- 本状态不进入权威 `GameState`、存档或服务端；领域业务组件只能通过通用事件创建草稿。

#### 12.1.3 抽屉语义与焦点

- 命令坞使用 `nav[aria-label="大地图命令坞"]`；领域按钮公开 `aria-expanded`、
  `aria-controls` 和稳定 `data-testid="command-domain-{domain}"`。
- 抽屉使用带名称的 `aside`；打开后焦点进入标题（无业务项时）或首个可操作项，关闭后返回原领域按钮。
- 抽屉标题、状态标记、可滚动正文、禁用原因区和底部动作区均由通用容器提供；领域仅注入内容，
  不复制布局或开关逻辑。
- 空态必须明确说明“此功能仍在原面板”或“设计中”，不得渲染能改变状态的占位按钮。
...
| **计略** | L1 美人计、离间、假情报、空城；**L2 十一计已齐（339~347）**；进行中计略、进度、分期投入、中止 | 白刃战/六角战术指令不放入大地图计略菜单；**L3 国策在朝廷（Session 348）** |
...
- 攻城产生 `pendingFamilyTreatment` 后，在命令坞之上显示全局 `FamilyTreatmentDialog`；显示失陷城、旧主、家属口数和受牵连城。
- 三个写按钮：`family-treatment-kindness`、`family-treatment-neutral`、`family-treatment-repression`；提交统一调用 store `resolveFamilyTreatment`，不在客户端计算效果。
- TopBar 的结束回合按钮在 `pendingEvents` 或 `pendingFamilyTreatment` 时禁用，并分别显示“待决事件…”或“待处置家属…”。
- 屯田抽屉仍负责迁家属与位置展示；城市行附带当前处置标签，避免新增第二套家属规则入口。

#### CMD-P40 家族直系族谱分面（Session 359）

- `FamilyOverviewDrawer` 由四分面扩为 `总览｜族谱｜姻亲｜婚配｜跟随`；“族谱”只读，不产生写操作或终审草稿。
...
- 朝廷抽屉直接读取当前 `GameState` 的玩家势力、君主、`politicalStage`、`politicalTitle`、
  `emperorLocation`、`imperialAuthority` 与 `imperialDecreeCooldown`；汉帝控制继续调用
  shared `controlsEmperor()`，不得在 UI 复制规则。
- “开霸府”和“伪诏宣战”先写入通用朝廷草稿，再进入既有 `CommandConfirmDialog`；取消终审
  保留草稿，成功后清草稿，服务端失败保留终审与参数。权威写动作继续唯一调用 store
  `establishHegemony()` / `falseDecreeWar()`，数值与规则不变。
- 伪诏使用单一目标选择器；诸侯、皇权不足、冷却中或已经交战均禁用送审并给出具体原因。
- 官制只读列出大司马、录尚书事、都督中外诸军事及当前任职者；“前往人事·任命”只打开旧
  人事手风琴，不复制任命表单或官职状态。
- CMD-P4 已结束 CMD-P2 验证期：旧“君主”折叠、开府/伪诏按钮及其终审 JSX 已从
  `LeftPanel` 真正删除，不通过 CSS 或功能开关隐藏。朝廷抽屉继续直接读写同一 Zustand
  `game` 权威快照，并成为该领域唯一入口。

#### 12.2.2 CMD-P5 复盘与下一迁移域

- Session 202 已完成朝廷域复盘，固定后续迁移 checklist：迁移前建立字段/action/Headless
  基线；接线期保持权威数据同源、跨域跳转不复制写逻辑；切换期同一提交删除旧入口并断言旧 DOM=0。
- 军事/人事/外交比较后，下一迁移域推荐**人事**，按
  `CMD-P6 基线 → P7 名册 → P8 招贤 → P9 任官/赏罚与跨域跳转 → P10 原子切换`
...
- Session 203 CMD-P6 已固化迁移前基线：人事抽屉分面为
  ...
  已实装的宫廷人脉笼络。朝廷官制跳转的目标语义是“人事·任官（霸府轨道意图）”，人物简册
  快捷入口只传 officer id 与分面意图，不复制表单。完整入口/action/错误断言见
  `docs/reviews/session-203-cmd-p6-personnel-baseline.md`。
- Session 204 CMD-P7 已把**只读名册**接入命令坞人事抽屉：固定显示
  `名册｜招贤｜任官｜赏罚`，本阶段只有“名册”可用，其余三项为 disabled 的迁移阶段标识，
  不创建假入口。名册从当前 `GameState` 派生己方在职与全局在野摘要，支持姓名筛选、
  在职/在野范围筛选及统帅/武力/智力/忠诚/姓名排序；列表拥有抽屉内独立滚动区。点击人物继续
  复用唯一 `OfficerDetail` 四页签，关闭后恢复至原名册行焦点。旧手风琴仍是搜索、任命、赏赐
  的唯一写入口，待 CMD-P8/P9/P10 逐步迁移。
- CMD-P7 大名单验证只使用开发态查询参数 `cmdP7RosterFixture=100|1000` 生成的内存合成夹具：
  不写入 `GameState`、不改服务端数据，也不代表 0-A 真实规模。1440×900 Headless 基准中，
  190 曹操初始真实名册为 5 人；合成 100/1000 行均保持唯一 key、无可视行重叠、列表独立滚动，
  首次打开至双帧渲染最近一次复验分别为 32.7ms/266.7ms（单机基线，非性能 SLA）。
- Session 205 CMD-P8 已启用“招贤”分面，将搜索人才和在野男将登用接入命令抽屉。城市选择与
  当前候选是客户端草稿；取消终审保留草稿，成功后清除。候选、支付能力及成功率仍从当前
  `GameState` 派生并调用 shared 公式，写操作只复用既有 store action/API。
- 招贤送审前显示无候选、无执行者、资源不足三类禁用原因；终审确认前重新读取最新 Zustand
...
- Session 215 CMD-P11 已建立外交迁移前基线：旧入口的进贡、献美、结盟继续是唯一外交
  写路径；后续抽屉按 `势力｜交涉｜盟约` 拆分。旧卡片中的“点化女间谍”权威端点属于
  S07 情报，迁移时只能跨域跳转，不得复制进外交表单。详见
  `reviews/session-215-cmd-p11-diplomacy-baseline.md`。
- Session 216 CMD-P12 已把外交“势力”只读分面接入命令坞：目标单选后展示关系、友好、
  君主、都城、领土与已知兵力；`交涉/盟约`保持禁用，新抽屉写按钮为0。旧入口实际进贡后
  新摘要即时读取同一权威友好值。详见 `reviews/session-216-cmd-p12-diplomacy-overview.md`。
- Session 217 CMD-P13 已启用外交“交涉”分面：进贡与献美复用原 store/API/引擎和统一
  终审，显示政治阶段友好增量、资源门槛与战争禁用原因。新旧两入口只作迁移期同源对照；
  “盟约”仍 disabled，点化仍无外交写入口。详见
  `reviews/session-217-cmd-p13-diplomacy-negotiation.md`。
- Session 218 CMD-P14 已启用“盟约·结盟”：显示 shared 同源成功率、使者、成本及四类
  禁用原因，终审列成功/失败后果并在确认前复验；停战/互不侵犯/求援/借道无假按钮。
  旧外交暂留至 P15 原子切换。详见 `reviews/session-218-cmd-p14-diplomacy-alliance.md`。
- Session 219 CMD-P15 已完成外交按域原子切换：`LeftPanel` 旧外交卡片与进贡/献美/结盟/
  点化按钮全部物理删除；命令坞 `势力｜交涉｜盟约` 成为外交唯一入口。“点化女间谍”
  迁入 `SpyPanel` 的“策反点化”正式区，只列有点化额度的存活目标并进入谍报统一终审。
  客户端不读取受 S06 裁剪的敌方宫廷人脉作门禁，该隐藏条件仍由服务端权威校验。
  详见 `reviews/session-219-cmd-p15-diplomacy-cutover.md`。
- Session 220 CMD-P16 已建立军事迁移前基线。军事现有入口分为右栏城池军备/简化出征、
  左栏 Campaign Army 战役链、S21 郡域与局部交战场景，不能整段搬运。后续抽屉固定为
  `军备｜编成｜军令｜战报`；P17 只接只读军情，新写按钮保持0。`marchOnCity` 简化出征与
  `campaignStart` 完整战役是两套生产链，P18 前必须完成归并决策。详见
  `reviews/session-220-cmd-p16-military-baseline.md`。
- Session 221 CMD-P17 已接入军事只读军情：`军备`展示17个己方城驻军/兵粮与汇总，
  `编成/军令`展示己方 Campaign Army 阶段、位置、兵粮、士气、组织与疲劳，`战报`读取既有
  `lastBattleResult/actionLog`。四分面均无业务 action 或提交按钮；旧战役写入后新摘要从
  同一 Zustand 快照即时刷新。旧战役与右栏军事仍是唯一写入口。详见
  `reviews/session-221-cmd-p17-military-overview.md`。
- Session 222 CMD-P18 已将 Campaign Army 定为唯一正式玩家出征路径。“军事·编成”
  提供出发城、主副将、参谋、目标、兵种、阵型、兵粮草稿与重大终审；取消保留草稿，
  确认前按最新状态复验，成功后生成战役军队。右栏“出征攻城”与左栏旧编成表单均物理
  删除；左栏军团后续指令留 P19。`/march` 仅作为 S21 场景兼容适配保留。
  详见 `reviews/session-222-cmd-p18-campaign-formation-cutover.md`。
- Session 223 CMD-P19 已将强攻、劝降、撤退、8 种营建和 4 种参谋行动迁入
  “军事·军令”，全部进入统一终审并按最新权威状态复验。左栏战役只保留军团只读详情与
  战报，旧军令写按钮物理归零；行军仍按既有回合自动推进，不新造提交按钮。
  详见 `reviews/session-223-cmd-p19-campaign-orders-cutover.md`。
- Session 224 CMD-P20 已将现有正式军备写链（征兵、训练）迁入“军事·军备”，显式选择
  己方城市并进入统一终审；确认前复验归属、金粮、成年男丁或最低驻军。右栏旧写按钮物理
  归零，只保留只读城情与命令坞导航。募兵、补充、整编、驻防、伤兵、复员仍无按钮。
  详见 `reviews/session-224-cmd-p20-military-readiness-cutover.md`。
- Session 225 CMD-P21 已完成军事原子总验收：`军备｜编成｜军令｜战报` 四分面各唯一，
  军备、Campaign 编成和 Campaign 军令均只在命令坞提交；展开右栏与左栏战役后旧写
  DOM=0。征兵、编成、激励三条取消/确认链及战报聚合已在 1440×900 浏览器实点通过。
  `/march` 继续仅作 S21 兼容，无玩家 UI。详见
  `reviews/session-225-cmd-p21-military-cutover-total.md`。
- Session 226 CMD-P22 已建立内政迁移前基线。右栏现有五个写按钮中，农业/商业/城防/
  施米属于 S03，寻访虽沿用 `/civil/seek-beauty`，业务真源属于 S09；征兵/训练已归军事，
  农业开发也不得冒充未实装屯田。五个旧按钮当前均点击即提交、没有统一终审，命令坞内政
  写按钮保持0。后续先做 `总览｜产业｜城建｜赈济` 只读壳，再迁 S03 四命令；寻访归属须在
  原子切换前拍板。详见 `reviews/session-226-cmd-p22-civil-baseline.md`。
- Session 227 CMD-P23 已将命令坞内政升级为可用只读抽屉：显式选择己方城市，并固定
  `总览｜产业｜城建｜赈济` 四分面。摘要直接读取当前权威快照；旧右栏操作后即时刷新。
  新写按钮保持0，S09 寻访不进入四分面。产业明确农业开发不等于屯田；城建明确当前
  `wall` 是城防开发度，不冒充未来修缮/耐久。详见
  `reviews/session-227-cmd-p23-civil-overview.md`。
- Session 228 CMD-P24 已将 S03 四条写链迁入对应分面：产业含农业/商业，城建含城防开发，
  赈济含施米。四条均以显式选城进入统一终审，展示当前资源、成本和收益范围，并在确认前
  复验最新归属与金粮。右栏 S03 写按钮物理归零，仅保留 S09 寻访；城防/修缮、农业/屯田
  语义边界不变。详见 `reviews/session-228-cmd-p24-civil-write-cutover.md`。
- Session 229 CMD-P25 经用户批准，在“内政·总览”加入明确标为 `S09 · 宫廷人脉` 的
  跨系统寻访卡片，不新增第五个内政分面，也不把寻访改归 S03。卡片显示势力库存、本城
  城市人脉机会与60金成本；提交进入统一终审，确认前按最新快照复验归属、机会与金钱。
  右栏寻访旧按钮物理删除，连同四个 S03 旧按钮总计旧写 DOM=0。详见
  `reviews/session-229-cmd-p25-civil-cutover-total.md`。
- Session 230 CMD-P26 已建立计略迁移前基线。迁移对象只包括 S17 `PlotPanel` 的美人计、
  离间计、假情报与空城疑兵；S07 探秘/女间谍只提供跨域前置，总军师任免继续归朝廷，
  白刃战/六角计策/Campaign 军令不进入大地图计略。后续抽屉固定为
  `态势｜发起｜进行中`：P27 只接同源只读摘要且新写按钮为0，P28 迁四计，P29 完成
  跨情报导航与旧 `PlotPanel` 原子下线。详见
  `reviews/session-230-cmd-p26-strategy-baseline.md`。
- Session 231 CMD-P27 已将命令坞计略升级为可用只读抽屉，固定
  `态势｜发起｜进行中`。态势从当前权威快照派生进行中计谋、势力资源、宫廷人脉、
  detailed 敌城、空闲女间谍与空城候选；发起只读列四计成本/前置；进行中只列己方计谋，
  不越权显示敌方计划。新提交入口保持0，旧 `PlotPanel` 写入后摘要即时同步；P28 前仍由
  旧面板唯一提交。详见 `reviews/session-231-cmd-p27-strategy-overview.md`。
- HC-P1-4“王命·封爵”展示受封者、旧爵→新爵、皇权与不可撤销，复用
  `CommandConfirmDialog danger` 深红重大终审（Enter 不触发确认），确认前读取最新状态复验。

Session 232 CMD-P28 已将 S17 四计草稿迁入“计略·发起”：按类型切换敌城/势力/己方
寡兵城目标，美人计可选空闲女间谍；不可提交时显示进行中上限、单城支付能力、情报、
美女、盟友或空城条件的具体原因。送交后复用统一终审，取消保留草稿，确认前按最新快照
再校验。迁移期旧 `PlotPanel` 仍保留一个写入口，P29 再连同跨情报导航一起原子下线。
详见 `reviews/session-232-cmd-p28-strategy-write-migration.md`。

Session 234 CMD-P29 已完成计略原子切换：“计略·发起”的“前往情报·探秘”通过命令壳
切换到 `intel/recon` 导航意图，不复制 S07 写链；左栏旧 `PlotPanel` 源码与 DOM 均归零。
四计现仅有命令坞一个提交入口。详见
`reviews/session-234-cmd-p29-strategy-cutover-total.md`。

Session 235 CMD-P30 已建立情报迁移前基线。迁移对象固定为 S07 普通招募、女间谍训练、
献美点化、派遣任务、驻守/撤回反间、俘虏处置；S17 四计、S08 献美、S09 寻访不复制。
后续情报抽屉固定 `态势｜人员｜任务｜反间`：P31 只读且新写入口0，P32 迁人员建设，
P33 迁任务/反间/俘虏，P34 原子下线旧 `SpyPanel` 并接稳 `intel/recon` 落点。详见
`reviews/session-235-cmd-p30-intel-baseline.md`。

Session 236 CMD-P31 已实装四分面只读抽屉：态势显示容量、可派遣、可见城情报及点化
额度；人员显示己方存活密探；任务只列己方日志；反间只列己方城防与己方扣押敌谍。
`intel/recon` 进入任务分面并显示迁移说明。本轮新写入口仍为0，旧 `SpyPanel` 保持唯一提交。
详见 `reviews/session-236-cmd-p31-intel-overview.md`。

Session 237 CMD-P32 已把普通招募、女间谍训练、献美点化迁入“人员”分面：显式选择
己方执行城，显示本次招募人数与成本，点化只列有额度的存续目标；三链统一终审并在确认前
复验最新可见状态。敌方宫廷人脉属于迷雾数据，不在客户端门禁，仍由服务端权威校验。
旧 `SpyPanel` 的三类人员建设控件归零，派遣/反间/俘虏留 P33。详见
`reviews/session-237-cmd-p32-intel-personnel-migration.md`。

Session 238 CMD-P33 已将五类任务派遣、反间驻防/撤防与俘虏处决/释放迁入“任务/反间”
分面。任务草稿显式选择空闲密探、任务和敌城，并显示官道邻接、盟友、女间谍专属及邻接
城支付门禁；反间显式选择己方城与空闲密探。所有命令进入统一终审并按最新状态复验。
旧 `SpyPanel` 现只剩只读名册，按钮与选择器归零；P34 再物理删除并做情报域原子总验收。
详见 `reviews/session-238-cmd-p33-intel-operations-migration.md`。

Session 239 CMD-P34 已物理删除旧 `SpyPanel` 与左栏谍报折叠项。Headless 总验收确认旧
源码/DOM/入口归零，人员三链、任务与反间入口唯一；并从“计略·发起”实际点击
“前往情报·探秘”，进入带 `intel/recon` 提示且可直接提交探秘的任务分面。情报域原子
切换完成。详见 `reviews/session-239-cmd-p34-intel-cutover-total.md`。
...
#### 12.4.1 终审焦点与键盘契约（Session 196 / CMD-P0 固化）

- 打开终审时保存触发元素；初始焦点落在“返回修改”，不得默认落在危险确认按钮。
- 焦点必须圈定在对话框内；关闭后优先回到抽屉“下达命令”，若触发元素已卸载则回到所属领域按钮。
- `Esc` 只关闭终审、返回抽屉并保留草稿；提交中禁用关闭、背景点击及重复确认。
- 背景不可操作。普通终审可点击遮罩返回；危险终审默认只能由按钮或 `Esc` 返回。
- `Enter` 不作为重大/不可逆命令的确认快捷键；只有明确聚焦确认按钮后的点击或 `Space` 激活才提交。
- 权威状态在抽屉打开后变化时，终审必须重新校验并以可见原因禁用确认，不能提交过期草稿。

#### 12.4.2 CMD-P3 运行时契约（Session 198）

- `CommandConfirmDialog` 是唯一终审组件；每个写操作通过 `validateBeforeConfirm()` 返回当前
  权威客户端 `GameState` 快照下的失效原因。组件在每次渲染和最终点击时复核：发现失效即以
  `role="alert"` 显示原因并禁用确认，不调用 store action/API。
- 每个终审标题采用“确认动作：关键对象”格式；标题 DOM ID 由 React `useId()` 按实例生成，
  `aria-labelledby` 不再复用全局固定 ID。
- 打开时初始焦点固定到“返回修改”；Tab/Shift+Tab 在终审内循环。`Esc` 以捕获阶段只关闭终审，
  防止同一次按键继续关闭底层抽屉；危险终审拦截 Enter。关闭后返回原触发器，触发器失效/
  禁用时可由调用方指定领域按钮作为回退焦点。
- 本次状态再校验复用现有 UI 门槛与 shared 查询函数，不新增服务端规则副本；服务端仍保留最终
  权威校验作为并发安全兜底。旧入口与朝廷新入口均读取同一 Zustand `game` 快照。

#### 12.4.3 HC-P1-5 朝廷称王流程（Session 211）

- “君主与政统”下方新增政治进程卡：诸侯显示下一步霸府；霸府阶段逐条显示城池、阶段沉淀月数、
  皇权的当前值/门槛与通过色，不把规则阈值写死在组件中。
- 霸府阶段从只读 `GET /hegemony/king-requirements` 取得有限王号候选；不可用王号标明占用，
  门槛未全过或未选可用王号时禁用“称王·送交重大终审”。
- 称王终审使用深红重大语义，列出王号、领土、沉淀、皇权80、解锁项和不可撤销；最终点击前以
  最新 `GameState` 重新检查阶段、城池、沉淀、皇权与其他存活势力王号，服务端再作最终校验。
- 官制总览在诸侯/霸府阶段只显示霸府三职；`king/emperor` 即时扩展为霸府三职+王国六职。
  任命写路径仍只跳转人事·任官·朝职，不在朝廷复制候选或提交逻辑。
- 称王成功后头衔、阶段、王国官职、王命封爵及外交 `+8/×1.2` 均从同一权威状态即时刷新。

**Session 191 旧手风琴安全补丁（已实装）**：在完整命令坞迁移前，现有行政 UI 已先复用
`CommandConfirmDialog` 为高风险入口补终审。覆盖外交（进贡、献美、点化女间谍、结盟）、
朝廷（开霸府、伪诏宣战）、军事（现为命令坞 Campaign 编成出征；战役强攻、劝降、撤退）、
计略发起、谍报（招募、女间谍训练、派遣、俘虏处决/释放）及不可撤销婚配。终审窗按操作
展示目标、明确资源数值、成功率/风险、关系或政治阶段后果；取消不调用 API 且保留表单参数，
服务端拒绝则保留窗口并显示错误。日常可重复的开发、征兵、训练、施米等操作，以及战斗内
逐步指令，不属于本次高风险安全补丁。

### 12.5 加强版“结束本月”

`进行`按钮永远需要终审，不允许单击立即推进。询问窗显示下一个月；若将进入 1/4/7/10 月，
额外标记“季度开始”，若跨年则同时标记“年度开始”。窗口汇总：
...
- 存储/载入属于系统菜单。Session 310 已接入真实浏览器文件层：顶部“导出存档”下载完整 JSON 信封，“导入存档”读取用户选择的 JSON 并交给服务端迁移/校验/恢复。Session 311/312 已补齐命名槽位 API 与顶部“槽位存档”面板：列表、保存、覆盖确认、读取确认和权威战局场景恢复均已接通。**Session 340**：服务端槽位落盘改为 `$XDG_DATA_HOME/leh/saves.db`（SQLite），UI 路径不变；多用户/云同步仍未实现。

---

## §13 独立郡域战场与场景栈（P1～P2 已实装）

### 13.1 场景层级

独立战场属于同一局游戏的内部场景，不新增浏览器路由。现有 Zustand `screen` 在 P2 升级为栈：

```typescript
type Scene = 'scenario' | 'world' | 'battlefield' | 'melee' | 'tactical' | 'duel';
...
world
  → battlefield（目标郡全境）
    → melee / tactical（一次 Encounter）
      → duel
```

单挑结束回接战、接战结束回郡域战场、战争结算后回大地图。浏览器刷新后的恢复目标由服务端进行中权威状态决定；镜头、hover、动画和临时选择不进入存档。

### 13.2 郡域战场画面

P1 南郡最小画面应包含：

- 郡治、经校勘属县、道路/水路、关渡及一个显式入侵入口；
- Army 位置、移动路径、节点控制/驻守状态和当前战争目标；
- 县城可占领/驻守并影响本场补给与结算，但不显示为完整全局经济城市；
- 历史模板信息与动态战况分层渲染；未侦察兵力仍按 S06 脱敏。

两军同节点、攻击驻军或强攻城池时弹出统一 Encounter 入口，提供"自动结算 / 标准模式 / 六角微操"。六角是可选局部战术层，不把整个郡铺成巨型六角图。

**BF-P2 Q9 县级攻打 UI（Session 176 已实装）**：`BattlefieldSceneView` 渲染 16 县节点 SVG——首批 3 县（当阳/华容/枝江）可点击（`cursor=pointer` + 棕色 `#5a4a2a` 区分非首批县灰色 `#3a3a32`）；占领后变绿色 `#2d5a2d` + 显示"驻N"文本（N=留驻兵力）；江陵 seat 保持红色 `#a21d24` + 金色边框。点击首批县触发 `engageCounty(countyId)` → 服务端 `runAutoBattle` 自动结算 → 节点状态流转（`rulerFactionId=attacker` / `garrison=remaining` / `controlTurns=0`）。
...
|------|---------|
| P0 | 只读南郡模板校验/预览；零玩法 |
| P1 | 静态郡域画面、节点行军、六角接战与逐层返回 |
| P2 | 正式场景栈、标准模式、多实例/撤退/僵持及刷新恢复；**Q9 县级攻打首批 3 县（当阳/华容/枝江）已实装** |
| P3 | 天气、部署、伏击/侦察与可复现战场 AI 表现 |
| P4 | 颍川对照模板、城下/阵前单挑入口 |
| P5～P6 | 核心战线批量覆盖、LOD/裁剪与全量模板性能 |

**BF-P3 UI（Session 250）**：南郡标题区显示天气、我方侦察、伏击方与部署军数；底栏显示
RNG 抽数区间。1440×900 实际进入南郡可见，console error=0。

模板未覆盖期间，开发构建的旧简化路径必须显著标注；正式内容不得静默退化为抽象随机战场。

BF-P4 当前切片在行政大地图右上提供“南郡水网 / 颍川平原”两个显式开发入口；郡域页标题、
郡治、入口县、县/路线计数均从实例派生。颍川实测显示 17 县/29 路线/阳翟，南郡保持
16 县/11 路线/江陵。颍川不误开放南郡首批县攻打按钮。Session 252 已在两郡统一加入
...
选择存活势力。皇权不足、冷却中或已交战时“送交终审”禁用，正文与 title 均附具体原因；
满足条件时进入统一终审窗，列明目标、皇权40、战争关系、8季冷却及可能的声望−30，确认后
才提交，仍不复用普通宣战前置。旧左栏“君主”入口已于 CMD-P4 删除。
...
- “结束回合”唯一入口为 TopBar。政治阶段、皇权、开霸府与伪诏宣战已在 CMD-P4
  迁至底部朝廷命令抽屉，左栏不再保留君主折叠项。

HC-P1-6 仓库化 Headless 固化 1440×900 验收：推进12个月后，朝廷进度必须显示
`17/8` 与 `12/12`；称王必须经过深红重大终审；成功后王国六职、关内侯与王号即时可见；
旧“君主”按钮数量必须为0。

---

#### 12.2.11 CMD-P35 家族迁移前基线

- 左栏 `FamilyPanel` 仍是唯一生产入口，现有三类只读内容为女眷、姻亲/固定子女、在野候选。
- 玩家写链仅婚配与手动跟随检查；前者已有统一终审，后者仍为即时提交。命令坞家族新写入口保持0。
- 后续固定 `总览｜姻亲｜婚配｜跟随`：P36 只读，P37 迁两条写链，P38 删除旧壳并做总验收。
- S09 宫廷人脉库存、人事搜索/登用、父辈族谱、随机出生、纳妾/离婚均不得混入本轮迁移。

#### 12.2.12 CMD-P36 家族四分面只读摘要

- 命令坞家族升级为可用，固定 `总览｜姻亲｜婚配｜跟随` 四分面。
- 总览与姻亲只展示玩家历史女角、剧本白名单固定子女和当前关系；婚配列可婚女角/无正妻
  武将；跟随列在野候选与已知触发条件，并明确邻接及权威 RNG 尚待结算。
- 新写入口保持0；旧 `FamilyPanel` 婚配/手动跟随各1且仍为唯一提交入口。
- P37 才迁两条写链；P35 的“随侍随迁”语义必须先拍板。

#### 12.2.13 CMD-P37 家族婚配/手动跟随写链迁移

- “婚配”分面提供女角与夫君草稿，进入危险终审；取消保留草稿，确认成功后清空。
- “跟随”分面提供手动检查普通终审，明确消费权威 RNG、可能无人投奔；取消不提交。
- 两条命令确认前均从最新 Zustand 快照复验；旧 `FamilyPanel` 写入口归零，仅留只读壳至 P38。
- 用户批准保留 `giftedToOfficerId` 随侍随迁；姻亲分面以“正妻/随侍”分别展示。

#### 12.2.14 CMD-P38 家族原子切换总验收

- 物理删除旧 `FamilyPanel.tsx` 与左栏家族折叠项；旧源码、DOM 与玩家入口归零。
- `FamilyOverviewDrawer` 成为 S18 女角、姻亲/固定子女、婚配与手动跟随唯一玩家入口。
- S11 加入/释放家眷同步与开局/年度固定子女仍是共享结算，不复制为 UI 命令。
...
### 12.2.19 S24 关系网 UI（Session 284；Session 299 标签归并）

- OfficerDetail 将家族与社交关系统一归入一个「关系」tab（与属性/装备/列传/技能并列），展示：
  - 家族关系：婚姻、妾/姬、子女、效力状态；
  - 社交关系列表：类型徽章（义兄弟/师徒/父子/挚友/宿敌等）+ 对象名 + 史源分色（正史绿/演义橙）+ 六等关系状态（亲密/友好/普通/嫌恶/仇敌）+ 亲和度数值
  - SVG 径向图谱：圆心=当前武将，第一圈=直接关系节点（按类型分色），悬停显示状态
- UI 不再并列提供「社交」页签；社交关系 API（S24）与家族数据（S18）仍保持职责分离，仅在武将详情页统一呈现。
...
- “撤销移动”仅在 `phase=player`、最后一条记录为 `source=player/kind=move/reversible=true`，
  且 `logicalTimestamp` 所属回合等于当前 `battle.turn` 时显示。按钮 title 明示本回合攻击、施法、结束行动
  或消耗 RNG 后的不可逆边界。
- BattleView 以 `move-path-summary` 显示服务端 A* 路径的格数、总消耗和剩余移动力；符合上述条件时显示
  `btn-battle-undo`“撤销移动”。交权后按钮立即隐藏，新玩家回合只在产生新移动后重现；六角格悬停通过同一服务端
  路径查询刷新摘要，客户端不自行计算路径或撤销合法性。
- Session 334 浏览器验收要求：同回合移动→撤销恢复；再移动→结束行动→敌军结算后旧按钮不出现；
  新回合新移动后按钮恢复且可撤销。直接端点请求还须返回 `UNDO_TURN_LOCKED` 并保持快照不变。
...
- 产业/城建分面启动项目时必须选择本城可用武将，终审显示首付、总成本、工期和完成收益。
- 总览显示当前项目状态、剩余月数、已付/总成本、暂停月数和累计进度损失。
- 总览显示未来12月预算：金收入减项目、行政、俸禄，以及粮产减民军耗粮；俸禄尚未
  实装时明确显示0。
- **Session 362 · 文化持续投入**：产业分面新增“文化积累”数值与“发展文化”命令；终审显示
  首付120金/总计360金/持续6个月/完成文化+60。总览的持续项目按中文“文化”显示，旧存档缺省
  文化值按0展示；本切片只展示与保存文化积累，不在客户端计算技术研发或人才吸引效果。
...
六角 BattleView 顶部状态栏同时显示 `BattleState.weather` 的中文天气、当前天气修正摘要和可选的
`weatherChangeTimer` 倒计时；该区域只读消费服务端快照，不在客户端重算天气或消费 RNG。雾天明确显示
远程禁射提示。

**Session 349**：主将为诸葛亮/司马懿且未行动、气力足够时，底栏出现「观天」
（`data-testid="btn-weather-skill"`）；展开后可选不同于当前的天气
（`battle-weather-picker` / `battle-weather-*`），提交走 `POST /battle/weather`。

**Session 350**：proficiency 战法按钮文案为「{名}·熟{N}」（N=`abilityUses`），leveled 战法仍只显示名称。
“远程禁射”，避免玩家将远程攻击失败误判为 UI 故障。

六角战斗右侧显示最近六条战报，并固定保留最近一次变阵解释，避免敌军回合和后续攻击将关键决策滚出面板：
变阵条目展示阵型前后与 TP 消耗，攻击条目展示攻守阵型贡献；服务端拒绝的变阵请求在同一面板显示阻断原因。
面板只读，不在客户端重算伤害或合法性。

Session 303 浏览器验收：`scripts/verify-fm4-report-ui.mjs` 通过 CDP 真实鼠标事件覆盖“选择我军→变阵→结束行动→移动→攻击”，并读取当前可用阵型以适配不同主将精通；验收要求面板同时出现变阵解释与攻击阵型贡献，且 console errors 为 0。

Session 308 浏览器验收：`scripts/verify-s10-enemy-duel-ui.mjs` 通过 CDP 真实进入六角战场，推进我军至敌军相邻格并点击“结束行动（敌军自动出手）”；检测到敌军主动单挑后，确认既有 `DuelPanel` 的倾向摘要、跳过按钮和结算态均可用，console errors 为 0。移动范围偶发刷新竞态仅由验收脚本调用同一权威移动接口回退，不改变生产 UI 或战斗规则。
- 已有项目时禁止再启动另一项；资源或人员导致暂停时由月结算权威更新状态。
