...
- 存储/载入属于系统菜单。Session 310 已接入真实浏览器文件层：顶部“导出存档”下载完整 JSON 信封，“导入存档”读取用户选择的 JSON 并交给服务端迁移/校验/恢复。Session 311/312 已补齐 XDG 命名槽位 API 与顶部“槽位存档”面板：列表、保存、覆盖确认、读取确认和权威战局场景恢复均已接通；SQLite、多用户/云同步仍未实现。
