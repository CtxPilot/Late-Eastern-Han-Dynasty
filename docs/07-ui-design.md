...
- 瓮城暂只在 `内政 → 城防工程`预留“未建/已建”与双重城门图标；战斗表现未拍板前不定义伤害、耐久或专属地图。
- 后续优先评估“外门 → 瓮城伏击 → 内城”的阶段式守城表现，但必须另行完成战斗规则设计后才能实装。
- 存储/载入属于系统菜单。S16 SQLite 未实现前仅显示“设计中”，不得制作可点击假存档入口。

---

*文档版本: v3.0 | 2026-07-19 | Session 106 实装ScenarioSelect、四势力选择、传奇开关与事件史源标签*
