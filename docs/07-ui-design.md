...
单挑主面板（弹窗·战术倾向 + 自动演算；当前 Demo 尚未接入倾向选择）：
┌──────────────────────────────────────────────┐
│  【单挑】        第3回合/上限10              │
│  关羽(蜀)    vs    夏侯惇(魏)                │
│  HP: ████████░░ 80%   ██████░░░░ 60%        │
...
- Session 219 CMD-P15 已完成外交按域原子切换：`LeftPanel` 旧外交卡片与进贡/献美/结盟/
  ...
  客户端不读取受 S06 裁剪的敌方美女库存作门禁，该隐藏条件仍由服务端权威校验。
...
- Session 229 CMD-P25 经用户批准，在“内政·总览”加入明确标为 `S09 · 宫廷人脉` 的
  ...
  可寻次数与60金成本；提交进入统一终审，确认前按最新快照复验城市归属、额度与金钱。
...
- Session 231 CMD-P27 已将命令坞计略升级为可用只读抽屉，固定
  `态势｜发起｜进行中`。态势从当前权威快照派生进行中计谋、势力资源、美女库存、
...
*文档版本: v6.3 | 2026-07-29 | Session 232 CMD-P28 计略四计写链迁移*
