...
**当前持久化边界（Session 340）**：已采用“完整保存进行中战斗 + 确定性续玩”方案。六角战斗、战场地图、白刃战均以 `GameState` 为权威真源；v1 信封保存 `xorshift32-v1` 的内部寄存器与消费计数。S15 军事目标、双线编成、撤退、自动战斗与伤亡均使用同一权威流，同保存点可复现；谍报/计谋 AI 的是否行动与目标选择仍保留各自决策边界，因此尚不承诺整个 AI 回合逐字节复现。**生产存取**：浏览器 JSON 导入/导出 + 系统菜单命名槽位；槽位介质为 `$XDG_DATA_HOME/leh/saves.db`（`better-sqlite3`），读档仍走迁移 → Schema → RNG 恢复；多用户/云同步未做。
