...
| 验证脚本对应 | `verify-save-battlefield-instance.ts` f6/f6b：真实路径判定断言（morale -5 + 粮耗×2 折算 + 仅守方受罚 + 对照不受罚），共享层单测见 `shared/army-county-mapping.test.ts`（加权路径/补给线判定/粮耗折算；Session 382 含代价分歧夹具）。 |
...
| 月度主动行动 | `shared/commandery-defender-ai.ts` 纯函数 `decideDefenderArmyAction(inst, army, ctx, rng)`，逐支守方 Army 在 `tickBattlefieldInstance` 内（补给线惩罚结算后）执行，优先级：① **所在县被攻方占领** → 兵力优势（`troops ≥ garrison×1.2`）且经 RNG 判定（60%~100% 概率）→ **收复**（`rulerFactionId` 夺回为守方、驻军并入）；劣势或判定失败 → **撤退**。② **补给线被切断** → 士气 <60 → 撤退；否则向 seat 移动一格（加权最短路径自 Army 侧上一跳；该步被攻方占则不移动，避免走进敌占县）。③ 存在攻方占领县 → 向**最近攻方占领县**移动一格（`totalCost` 最小路径首步；并列比跳数）。④ 否则原地驻守。 |
