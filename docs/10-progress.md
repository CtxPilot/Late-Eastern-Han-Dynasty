...
## 2026-08-05 — Session 331 · S10 敌军 AI 完全阻挡待机契约收口

- Phase：**S10 战斗深化**；继续同一大系统，不扩 0-B、不新增 BattleState 字段、API、RNG 或数据规模。
- 修复：敌军单位若被其他存活单位完全阻挡、找不到可达格，现记录“无法接近目标，待机”，并统一写入 `hasActed=true`、`mp=0`；重复调用遵守既有重入门禁，返回“敌军待机”，不重复寻路或消费攻击 RNG。
- 文档：同步 `docs/05-combat-system.md` §18.1.1 与根目录 `HANDOFF.md`。
- 验证：`verify-tactical-ai` **43/43**（新增完全阻挡、行动状态、重入与 RNG 断言）；`verify-save-battle` **36/36**；server typecheck、shared build、`git diff --check` 通过。
- 边界（诚实）：天气主动切换技能、地形可见范围、移动后特殊连击、正式特殊兵种熟练度、0-B 数据、阵型协同包围/撤退仍未完成；本轮不宣称完整 S10 或 P5-01 全局 AI。
- **Next**：保持 S10，继续玩家/敌军战法、天气与行动状态的权威一致性回归；天气主动技能仍需用户确认正式规则后再做。

*v16.11 | 2026-08-05 | Session 331 · S10 敌军 AI 完全阻挡待机契约收口*

## 2026-08-05 — Session 332 · S10 敌军 AI 异常快照待机契约收口

- Phase：**S10 战斗深化**；继续同一大系统，不扩 0-B、不新增 BattleState 字段、API、RNG 或数据规模。
- 修复：敌军 AI 遇到缺失目标或兵种模板的异常战斗快照时，统一记录异常待机并写入 `hasActed=true`、`mp=0`，不消费攻击 RNG；后续重入遵守既有行动门禁，返回“敌军待机”。
- 文档：同步 `docs/05-combat-system.md` §18.1.1 与根目录 `HANDOFF.md`。
- 验证：`verify-tactical-ai` **45/45**（新增缺失兵种模板、行动状态与 RNG 断言）；`verify-save-battle` **36/36**；server typecheck、shared build、`git diff --check` 通过。
- 边界（诚实）：天气主动切换技能、地形可见范围、移动后特殊连击、正式特殊兵种熟练度、0-B 数据、阵型协同包围/撤退仍未完成；本轮不宣称完整 S10 或 P5-01 全局 AI。
- **Next**：保持 S10，继续玩家/敌军战法、天气与行动状态的权威一致性回归；天气主动技能仍需用户确认正式规则后再做。

*v16.12 | 2026-08-05 | Session 332 · S10 敌军 AI 异常快照待机契约收口*

## 2026-08-09 — Session 333 · S10 敌军跨回合行动窗口修复

- Phase：**S10 战斗深化**；继续同一大系统，不扩 0-B、不新增 BattleState 字段、API、数据规模或随机源。
- 根因：敌军行动后会写入 `hasActed=true`，但玩家交回控制权时从未恢复存活敌军的行动资格；因此首个敌军阶段后，后续回合永久只返回“敌军待机”。
- 修复：新增内部 `enterEnemyPhase` 边界，普攻、火计、战法和“结束行动”四条玩家交权路径统一恢复存活敌军 `hasActed=false`，并按当前天气恢复有效 MP；同一敌军阶段的重入门禁保持不变。
- 天气一致性：新增 `hasMovedThisTurn`，玩家与敌军普攻均以天气修正后的回合初始 MP 为“是否移动”基线；雨/雪天未移动单位不再误触发强行军连击加成，实际移动后仍正常生效。
- 功能验证：`verify-save-battle` **42/42**，实际跑通“首轮敌军攻击 → 玩家交回回合 → 雪天有效 MP 恢复 → 第二轮敌军再次攻击”并确认两轮均消费权威 RNG；`verify-tactical-ai` **49/49**，含雪天原地/移动后强行军对照。
- 扩大回归：`verify-battle-rng` **5/5**、`verify-fm4-hex-formation` **14/14**、shared **378/378**、server typecheck/lint、`git diff --check` 全部通过。
- 边界（诚实）：跨回合移动撤销封闭、单挑期间战场暂停门禁、敌军主动单挑结算后继续敌军阶段，以及旧/外部雪天快照的移动预算夹紧仍未完成；天气主动技能和正式特殊兵种熟练度仍待用户批准。
- **Next**：保持 S10，优先封闭跨回合移动撤销，再分别收口单挑暂停门禁与敌军主动单挑的敌军阶段续行；不并行启动新大系统。

*v16.13 | 2026-08-09 | Session 333 · S10 敌军跨回合行动窗口修复*

## 2026-08-09 — Session 334 · S10 移动撤销回合边界收口

- Phase：**S10 战斗深化 / UI 收口**；继续同一大系统，不扩 0-B，不新增 BattleState 字段、端点、数据规模或随机源。
- 根因：移动审计记录在敌军阶段与新回合仍保留 `reversible=true`，服务端和 BattleView 又都未校验
  `logicalTimestamp` 所属回合，因此能在敌军结算后用旧记录回滚坐标与 MP。
- 修复：所有 `player → enemy` 交权路径统一封闭可逆记录，但保留最近3条审计历史；新增共享
  `tacticalTurnFromTimestamp`，服务端以 `source/kind/reversible/current turn` 作为撤销门禁，跨回合返回
  `UNDO_TURN_LOCKED`。
- 状态不变式：撤销前复验单位为存活攻方、当前落点匹配 `afterPosition`、原位置在界内且未被占用、
  `beforeMp ≤ maxMp`；伪造/不一致记录返回 `UNDO_STATE_MISMATCH`，拒绝时不改写坐标、MP、历史或战报。
- UI：`btn-battle-undo` 只在当前玩家回合的最后一条玩家移动仍可逆时显示；交权后隐藏，下一回合只在产生
  新移动后重现。按钮 title 同步明示攻击、施法、结束行动与 RNG 边界。
- 旧档兼容：不改 Schema/存档版本；修复前 stale `reversible=true` 记录仍能严格解析，但在运行时被降权拒绝。
- 功能验证：`verify-save-battle` **47/47**，实际跑通同回合撤销、交权封闭、敌军结算、新回合拒绝、旧档解析与
  伪造落点拒绝；Headless Chrome 1440×900 实际点击通过“同回合移动→撤销→再移动→结束行动→新回合旧按钮消失/
  端点返回 `UNDO_TURN_LOCKED`→新移动再可撤”，`consoleErrors=0`。
- 扩大回归：`verify-tactical-ai` **49/49**、`verify-battle-rng` **5/5**、`verify-fm4-hex-formation` **14/14**、
  shared **378/378**、client **43/43**、全仓 typecheck/lint 通过。
- 边界（诚实）：本轮完成跨回合封闭与位置/MP 权威复验；移动撤销尚未恢复移动前 `facing`，审计历史达3条后
  同回合继续写入的 sequence/ID 仍可重复。两者不影响本轮回合门禁，但不得宣称撤销/审计已全量完成。
- **Next**：保持 S10，优先收口单挑期间的战场暂停门禁，再处理敌军主动单挑结算后的敌军阶段续行；撤销朝向恢复与
  审计序号唯一性作为相邻技术债跟进，不并行启动新大系统。

*v16.14 | 2026-08-09 | Session 334 · S10 移动撤销回合边界收口*
