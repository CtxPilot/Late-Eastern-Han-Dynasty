...
> 为避免与项目总路线 Phase 0～5 混淆，工程任务 ID 使用 `BF-Px`；设计文档仍简称 P0～P6。顺序与门禁以 `docs/21-battlefield-scene-design.md` 为准，当前下一步只启动 BF-P0。

| ID | 阶段 | 主要产出 | 验收门禁 |
|:--:|------|---------|---------|
| BF-P0 | 资料与 Schema 契约 | 明确剧本年份；南郡基线+年代覆写；Commandery/County/Route/Landmark Zod、来源与只读预览 | 逐条可追溯；引用/有效期/坐标/连通性通过；静态预览零 RNG |
| BF-P1 | 静态郡域战场 + 既有六角 | 大地图进入南郡；县节点行军；局部六角接战；结果回写 | 实际走通出征→郡域→接战→占/守江陵→大地图；伤亡与控制只结算一次 |
| BF-P2 | 实例管理 + 标准模式 + 场景栈 | `BattlefieldInstance`/`Encounter`；三入口统一；多实例、撤退/僵持、中途恢复 | 场景逐层返回；同一 Army 不跨两战场；刷新/读档恢复进行中状态 |
| BF-P3 | 动态战况与权威 RNG | 天气、部署、伏击/侦察、遭遇顺序、战场 AI、生成审计 | **AI 行动选择也接入权威 `xorshift32-v1`，同 seed 整场复现；保存点续玩一致** |
...
*文档版本: v2.1 | 2026-07-23 | Session 162 批准独立战场 BF-P0～P6 子路线*
