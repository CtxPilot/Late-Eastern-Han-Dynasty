...
| BF-P3 | 动态战况与权威 RNG | 天气、部署、伏击/侦察、遭遇顺序、战场 AI、生成审计 | **AI 行动选择也接入权威 `xorshift32-v1`，同 seed 整场复现；保存点续玩一致**。**范围边界（Q12 已声明）：**仅覆盖出征/围城/自动战斗层面的既有决策 RNG 收口，**不含县级战术目标选择**（AI 主动攻县归 R6 多线 AI 范畴，与 BF-P3 解耦）。**注（Session 177）：**"伏击/侦察"当前仅指 Tier I 出征/围城层面；Tier II 郡域场景的迷雾层（mask 下沉到 `BattlefieldInstance`）尚未设计，是新发现缺口（见下条 BF-P5 与 `docs/23-design-consistency-remediation.md` R6 补登记），不归属 BF-P3 |
| BF-P4 | 第二郡地形对照 | 颍川郡；陆地/平原模板；城下/阵前单挑 | 两郡差异可验证；生成器无郡名硬编码；旧模板可加载/显式迁移 |
| BF-P5 | 核心战线批量扩展 | 录入/校勘工具；年代覆写；县控接补给与郡国归属 | 目标剧本可达交战郡均有模板；无静默抽象回退。**待办依赖（Session 177 补登记，源自 BF-P2 Q9 老实标注）：**(1) 补给线糧耗×2 真实路径判定——BF-P2 当前用"占领任意首批县→守方全军 morale -5"全局简化替代（见 `docs/25-bf-p2-design.md` §2.6.1），真正路径判定需先解决 `CampaignArmy`（数字 cityId）与郡域县节点（字符串 countyId）的位置映射；(2) **郡域场景迷雾机制——新发现缺口**，此前从未被任何阶段排期覆盖（BF-P0~P2 未做、S06 迷雾只服务 Tier I、BF-P3"伏击/侦察"只覆盖 Tier I），需作为新缺口正式登记，落地后才能做"占领县→揭示周边敌方 Army"的视野扩张效果（BF-P2 Q9 的视野扩张攻占效果当前仅为占位视觉反馈，见 `docs/25-bf-p2-design.md` §2.6.2）。两项均不归属 BF-P3，倾向随 R6（S15 多线 AI）或 BF-P5 排期 |
...
| HC-P1 | 称王 + 王国官职 | **实施中，HC-P1-1～3 已完成**：相对城池门槛与阶段年龄→原子称王/固定王号→王国六职均已落地；下一项 HC-P1-4 为王命封爵与爵位语义收口，随后 P1-5 外交分档+朝廷 UI、P1-6 总验收。K1～K8 已拍板，见 `28-hc-p1-king-design.md` |
...
*文档版本: v2.5 | 2026-07-24 | Session 177 同步 BF-P2 Q9 老实标注（补给线/视野扩张为简化替代或占位）+ R6/BF-P5 补登记两项待办依赖（含郡域迷雾新缺口）*
