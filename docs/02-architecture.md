...
边界：**Session 376 起全部 114 个接口离线覆盖**（372 基础闭环 → 374 Tier I 战场+白刃战+kingRequirements → 375 总军师/技能树/关系网/势力总览等 14 接口 → 376 郡域实例写链 ×7），网关不再有在线回退面。**PWA 预缓存已实装（Session 373）**：构建期生成 `sw.js` 预缓存全量产物（清单哈希命名缓存），导航网络优先回退缓存首页，`/api/*` 放行——Pages 版首访后支持完全离线冷启动。**Worker 启动注入（Session 374）**：镜像 `server/index.ts` 注入五处阵型目录/TacticalConfig v2，缺失时引擎会静默回退中性值清零战斗数值。
