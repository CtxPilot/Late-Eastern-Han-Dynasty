...
/**
 * S03 Session 362 · 文化持续投入专项验收。
 * 运行：pnpm verify-culture-development
 */
import {
  GameStateSchema,
...
console.log('Culture development verify');
...
const beforeCulture = city.stats.culture ?? 0;
const beforeGold = city.gold;
const beforeDraws = getRuntimeRngState().draws;
const config = DEVELOPMENT_PROJECT_CONFIG.culture;

state = developCity(state, city.id, 'culture', officerId);
const started = state.cities[city.id]!;
assert(started.activeDevelopment?.kind === 'culture', '启动文化持续项目');
...
assert((started.stats.culture ?? 0) === beforeCulture, '启动不提前增加文化');
...
assert((finished.stats.culture ?? 0) === beforeCulture + config.gain, '完成后文化 +60');
assert(finished.gold < beforeGold, '月结持续扣除余款');
assert(getRuntimeRngState().draws === beforeDraws, '文化项目全程不消费 RNG');
