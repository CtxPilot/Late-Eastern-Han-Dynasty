...
/**
 * S03 Session 397 · 工艺持续投入专项验收。
 * 运行：pnpm verify-craft-development
 */
import {
  GameStateSchema,
...
console.log('Craft development verify');
...
const beforeCraft = city.stats.craft ?? 0;
const beforeGold = city.gold;
const beforeDraws = getRuntimeRngState().draws;
const config = DEVELOPMENT_PROJECT_CONFIG.craft;

state = developCity(state, city.id, 'craft', officerId);
const started = state.cities[city.id]!;
assert(started.activeDevelopment?.kind === 'craft', '启动工艺持续项目');
...
assert((started.stats.craft ?? 0) === beforeCraft, '启动不提前增加工艺');
...
assert((finished.stats.craft ?? 0) === beforeCraft + config.gain, '完成后工艺 +60');
assert(finished.gold < beforeGold, '月结持续扣除余款');
assert(getRuntimeRngState().draws === beforeDraws, '工艺项目全程不消费 RNG');
