...
/**
 * S03 Session 399 · 卫生持续投入专项验收。
 * 运行：pnpm verify-sanitation-development
 */
import {
  GameStateSchema,
...
console.log('Sanitation development verify');
...
const beforeSanitation = city.stats.sanitation ?? 0;
const beforeGold = city.gold;
const beforeDraws = getRuntimeRngState().draws;
const config = DEVELOPMENT_PROJECT_CONFIG.sanitation;

state = developCity(state, city.id, 'sanitation', officerId);
const started = state.cities[city.id]!;
assert(started.activeDevelopment?.kind === 'sanitation', '启动卫生持续项目');
...
assert((started.stats.sanitation ?? 0) === beforeSanitation, '启动不提前增加卫生');
...
assert((finished.stats.sanitation ?? 0) === beforeSanitation + config.gain, '完成后卫生 +60');
assert(finished.gold < beforeGold, '月结持续扣除余款');
assert(getRuntimeRngState().draws === beforeDraws, '卫生项目全程不消费 RNG');
