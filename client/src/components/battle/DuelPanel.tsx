...
export function DuelPanel({ duel, onStep, onSkip, onClose, resolveName }: Props) {
  ...
  return <div className="absolute inset-0 z-30 bg-black/80 flex items-center justify-center">
    <div className="w-[680px] max-w-[95%] rounded-xl border-2 border-amber-700 bg-stone-950 p-5 shadow-2xl">
      ...
      <div className="flex items-center justify-between gap-2"><div className="flex gap-1">{(['full', 'fast', 'skip'] as const).map((m) => <button key={m} disabled={resolved} className={`px-2 py-1 rounded border text-xs ${mode === m ? 'bg-amber-700 border-amber-400 text-amber-50' : 'bg-stone-800 border-stone-600 text-stone-300'}`} onClick={() => setMode(m)}>{m === 'full' ? '观看演出' : m === 'fast' ? '快速结算' : '只看结果'}</button>)}</div><div className="flex gap-1">{!resolved && <><button data-testid="btn-duel-step" className="px-3 py-1.5 rounded bg-emerald-900 border border-emerald-600 text-sm" onClick={() => void step()}>下一回合 →</button><button data-testid="btn-duel-skip" className="px-3 py-1.5 rounded bg-stone-800 border border-stone-600 text-sm" onClick={() => void skip()}>跳过 ▶▶</button></>}{resolved && onClose && <button data-testid="btn-close-battlefield-duel" className="px-3 py-1.5 rounded bg-amber-900 border border-amber-600 text-sm" onClick={() => void onClose()}>返回战场</button>}</div></div>
    </div>
  </div>;
}
