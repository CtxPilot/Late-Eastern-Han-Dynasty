...
export function BattleView() {
  ...
  const surroundedAttackerCount = activeUnits.filter((unit) => unit.side === 'attacker' && resolveHexSurround(battle.units, unit.id).isSurrounded).length;
  const anyAttackerSurrounded = surroundedAttackerCount > 0;
...
function battleErrorText(value: string) {
  if (value.startsWith('RETREAT_SURROUNDED:')) {
    const names = value.slice('RETREAT_SURROUNDED:'.length).trim();
    return names ? `被协同包围，无法撤退（${names}）` : '被协同包围，无法撤退';
  }
  return value.includes('（') ? blockReason(value.slice(value.indexOf('（') + 1, value.lastIndexOf('）'))) : value;
}
