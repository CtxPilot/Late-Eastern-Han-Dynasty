    ...
    <div className="px-2 pb-2 text-[11px]" data-testid="officer-roster-panel">
      ...
      <div className="max-h-64 space-y-1 overflow-y-auto">
        {officers.map((officer) => {
          ...
          return <button key={officer.id} type="button" data-testid={`officer-row-${officer.id}`} onClick={() => setSelected(officer)} className={`w-full rounded border bg-stone-900/60 px-2 py-2 text-left hover:bg-amber-950/20 ${lowLoyalty ? 'border-red-600' : 'border-stone-800 hover:border-amber-800'}`}>
            <div className="flex items-center justify-between"><strong className="text-stone-100">{officer.name}</strong><span className={lowLoyalty ? 'text-red-300' : 'text-stone-500'}>忠 {officer.loyalty}</span></div>
            <div className="mt-1 flex justify-between text-[10px] text-stone-500"><span>统{officer.stats.leadership} · 武{officer.stats.war} · 智{officer.stats.intelligence}</span><span>{officer.location != null ? game.cities[officer.location]?.name ?? '未知' : '未驻城'} · {officer.status === OfficerStatus.ACTIVE ? '在职' : String(officer.status)}</span></div>
          </button>;
        })}
      </div>
