    ...
    <div className="space-y-4 p-4">
      ...
      <section>
        ...
        <div className="rounded border border-stone-800 bg-stone-900/50 p-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="flex items-baseline justify-between mb-1">
                <span className="text-xs text-stone-500">声望</span>
                <span className={`text-sm font-bold ${fameColor(data.fame)}`}>{data.fame}<span className="text-xs text-stone-500">/1000</span></span>
              </div>
              <p className="text-xs text-stone-400">{data.fameLabel}</p>
              <p className="text-xs text-stone-500">≥300/600/900 投奔概率 +10%/+20%/+35%；每季 −2</p>
            </div>
            <div>
              <div className="flex items-baseline justify-between mb-1">
                <span className="text-xs text-stone-500">兵装</span>
                <span className="text-sm font-bold text-stone-200">{data.arms}<span className="text-xs text-stone-500">件</span></span>
              </div>
              <p className="text-xs text-stone-500">月产首都+8、城防≥150再+2；满配战力+5%、缺口过半−10%</p>
            </div>
          </div>
          <InkButton
            type="button"
          ...
          >
            采购兵装 ×10
            <span className="mt-0.5 block text-xs text-stone-500">100金（10金/件）</span>
          </InkButton>
        </div>
      </section>
      <InkButton type="button" onClick={onClose} className="w-full px-3 py-1.5 rounded border border-stone-700 text-stone-400 text-xs hover:text-stone-200">关闭</InkButton>
    </div>
  );
}
