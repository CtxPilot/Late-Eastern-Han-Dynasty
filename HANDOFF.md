...
| 项 | 状态 |
|----|------|
| 会话 | **Session 156 完成**（总军师/幕僚确定性结算收口） |
...
| 文档最新 | 本文件 · 10-progress v10.5 · **20-architecture-hardening-audit（Gate 2 PRNG：非 S15 玩家/共享结算收口完成）** · 19-design-proposal-templates · 18-learning-guide · README 框架定位 · 12-system-map v7.6 · 05-combat-system v4.5 · 03-data-models v4.3 · 04-game-systems §3.5/§14/§30.7/§37 · 01-overview v2.6 · 02-architecture v2.5 · 06-api-design v2.6 · 07-ui-design v3.6 · 09-roadmap v2.0 · 08-data-dictionary v2.2 · **14-officer-stats-reference v2.0** |
...
| S16 | 剧本/存档 | **M/D** | v1 信封、完整 Schema/跨引用、迁移、受锁内存恢复及可序列化 `xorshift32-v1` 已实装；非 S15 玩家/共享结算 RNG 已收口；S15 决策、生产存取与 SQLite 仍未做 |
...
- `pnpm test` 当前跑 **shared 层12个测试文件、112项契约/纯函数测试**；默认 CI 另跑既有战役/存档域检查，以及 **战斗（5）**、**单挑（3）**、**内政（12）**、**计谋谍报（30）**、**人事（32）**、**家族（32）**、**美女资源（25）**、**总军师（28）** 确定续玩检查与数据校验。暴击/反击/连击、火计、子女等专用脚本仍需按改动范围单独运行，不代表全仓端到端覆盖。
...
| 优先级 | 事项 |
|:------:|------|
| **1** | 非 S15 玩家/共享结算随机收口已完成；下一独立任务统一审计 `aiMilitary.ts` / `plotAi.ts` / `spyAi.ts` 的 S15 AI 决策边界 |
...
*Session 156 交接 | 2026-07-22 | 总军师/幕僚 RNG 收口完成：献策/辞职确定续玩 28/28；直接 Math.random 4→3 且仅剩 S15 AI 决策文件；非 S15 结算收口阶段完成 | 未 push*
