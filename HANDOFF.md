...
| 项 | 状态 |
|----|------|
| 会话 | **Session 159 完成**（S06 迷雾投影与出征资格解耦） |
| 阶段 | Phase 0-A + Demo 玩法环；**暂缓 0-B**；系统数 **22 大** |
| 代码最新 | **S06 未探明目标可尝试道路邻接出征（资格不读取目标脱敏归属/兵力，服务端权威校验）** + 两剧本选择 + 190**24事件沉浸式链**/5叙事线/玩家抉择系统/史源分层 + 9兵种 + 战法/单挑/暴击反击连击引擎 + 战役层引擎 + 跨平台字体防御 + **223名史实武将** + **武将名册/人物简册/四名代表人物程序化头像/README四图展示/人事终审窗** + **三层战斗架构实装（战场地图画面+白刃战标准模式引擎+战术点系统；三级状态均进入 GameState）** + **总军师系统实装（任命/态势切换/献策/对决/加成接入）** |
| 文档最新 | 本文件 · 10-progress v10.8 · 04-game-systems v4.7 · 06-api-design v2.9 · 07-ui-design v3.6 · **20-architecture-hardening-audit（Gate 2 RNG 阶段性完成）** · 12-system-map v7.8 · 05-combat-system v4.5 · 03-data-models v4.3 · 01-overview v2.6 · 02-architecture（Session 157 边界）· 09-roadmap v2.0 · 08-data-dictionary v2.2 |
...
| S15 | AI | M+ | 内政占位 + 出征占城 + **总军师自动态势切换**；行动后结算接权威 RNG，行动/目标选择保留独立随机 |
...
- `pnpm test` 当前跑 **shared 层12个测试文件、112项契约/纯函数测试**；默认 CI 另跑既有战役/存档域检查，以及 **战斗（5）**、**单挑（3）**、**内政（12）**、**计谋谍报（30）**、**人事（32）**、**家族（32）**、**美女资源（25）**、**总军师（28）**、**AI军事结算（7）** 确定续玩检查与数据校验。暴击/反击/连击、火计、子女等专用脚本仍需按改动范围单独运行，不代表全仓端到端覆盖。
...
| 优先级 | 事项 |
|:------:|------|
| **1** | 恢复玩法队列：势力特点数据 → **委任军团引擎实装（§39 设计完成）** → AI Army 接入 → 战略点系统 |
| **已知后续（本轮未做）** | 初始驻军差异化配置（小~中改动）与 S15 AI 军事决策/AI Army 完善（中等改动）须分别设计、平衡并独立提交 |
| **旁路缺陷** | 未探明目标进入六角战斗后，敌方武将名显示 `undefined`；战斗创建/结算正常。应从战斗 DTO/表现层补足必要身份信息，不得放宽大地图 S06 裁剪 |
...
*Session 159 交接 | 2026-07-23 | S06 迷雾投影与出征资格解耦；驻军差异化与 AI 军事完善明确后置*
