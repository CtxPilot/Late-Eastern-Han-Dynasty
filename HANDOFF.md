...
| 项 | 状态 |
|----|------|
| 会话 | **Session 163 完成**（BF-P0 南郡历史地理 Schema 与首批校勘） |
...
| 文档最新 | 本文件 · 10-progress v11.2 · **22-nanjun-historical-geography-collation（BF-P0）** · 21-battlefield-scene-design（P0 完成） · 08-data-dictionary 南郡规模真源 · 03-data-models 静态契约实装边界 · CREDITS 正式史料条目 |
| 本交接用途 | 190四槽技术切片已可运行；0-A验收基线仍为30武将，当前武将数据223人；0-B继续暂缓 |
| 玩法下一步 | **等待用户审阅 BF-P0 后启动 BF-P1**。P1 前须拍板/明确处理：190 南郡模板包含襄阳而世界地图另有襄阳节点；江陵旧 `countyCount: 7` 与校勘 17 县不一致。驻军差异化仍排在独立战场之后 |
...
- **BF-P0 静态地理**：`shared/data/historical-geography/` 已有严格 Zod、南郡 190 年
  1 郡/17 县/12 路线/10 地标与零 RNG 只读预览；未接任何运行时。字段级校勘见
