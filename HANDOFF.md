...
| 项 | 状态 |
|----|------|
| 会话 | **Session 167 完成**（GitHub 人物简册展示改为可读的 2×2 大图并刷新缓存） |
| 阶段 | Phase 0-A + Demo 玩法环；**暂缓 0-B**；系统数 **22 大** |
| 代码最新 | **S15 外交过滤+君主激进度+AI CampaignArmy 出征/围城/自动结算+战报（军事决策与结算统一权威 PRNG）** + **S10 正式交战单位携带主将姓名快照** + **S06 未探明目标可尝试道路邻接出征** + 两剧本选择 + 190**24事件沉浸式链**/5叙事线/玩家抉择系统/史源分层 + 9兵种 + 战法/单挑/暴击反击连击引擎 + 战役层引擎 + 跨平台字体防御 + **223名史实武将** + **武将名册/人物简册/四名代表人物金石水墨位图头像（其余程序化回退）/README四图展示/人事终审窗** + **三层战斗架构实装** + **总军师系统实装** |
| 文档最新 | 本文件 · 10-progress v11.4 · 09-roadmap v2.2 · 12-system-map v8.1 · **22-nanjun-historical-geography-collation（两项冲突已决议）** · 21-battlefield-scene-design（战略节点/史实郡县分离原则） · 08-data-dictionary 南郡规模真源 · 03-data-models countyCount 语义 · CREDITS 正式史料条目 |
...
| 优先级 | 事项 |
|:------:|------|
| **1** | 等用户派发 **BF-P1 静态郡域闭环**详细 prompt；BF-P0 Schema、南郡校勘及两项数据冲突均已完成 |
...
*Session 167 交接 | 2026-07-23 | GitHub 人物简册整合展示已放大并刷新缓存；下一步等待 BF-P1 详细任务*
