...
| 项 | 状态 |
|----|------|
| 会话 | **Session 123 完成**（三层战斗架构设计定稿：战场地图层 + 白刃战层 + 战术点系统） |
| 阶段 | Phase 0-A + Demo 玩法环；**暂缓 0-B**；系统数 **22 大** |
| 代码最新 | **两剧本选择** + 190**24事件沉浸式链**/5叙事线/玩家抉择系统/史源分层 + 9兵种 + 战法/单挑/暴击反击连击引擎 + 战役层引擎 + 跨平台字体防御 + **223名史实武将** + **武将名册/详情/人事终审窗** |
| 文档最新 | 本文件 · 10-progress v7.0 · 12-system-map v5.5 · 05-combat-system v4.3 (§二十 三层战斗架构) · 03-data-models v2.9 · 04-game-systems v4.7 · 01-overview v2.6 · 02-architecture v2.4 · 06-api-design v2.5 · 07-ui-design v3.4 · 09-roadmap v2.0 · 08-data-dictionary v2.2 · **14-officer-stats-reference v2.0** |
| 本交接用途 | 190四槽技术切片已可运行；0-A验收基线仍为30武将，当前武将数据223人；0-B继续暂缓 |
| 玩法下一步 | **三层战斗架构实装**（战场地图画面 + 标准模式 runMeleeRound 引擎 + 战术点系统）为当前优先路线 → 总军师系统态势接入战场地图层 → 设施建造回合化 → 势点系数据 → 委任军团引擎实装（§39 设计完成）→ AI Army 接入。**S22 Linux UI 适配 + 武将头像 A+C+B 留 Phase 5** |
...
| S10 | 战斗 | **M+引擎/入口有缺陷** | 六角战斗、单挑引擎、暴击/反击/连击与战役 Army 编成/行军/围城/自动战斗已有实现；**单挑接受链路有嵌套锁400错误**，战役单挑也只是简化事件。**已设计**：阵型双轴成长体系（§4, Session 120）+ 工程器械6种Lv1~Lv3+城防体系+瓮城阶段式守城（§15, Session 121）+ **三层战斗架构（§20, Session 123）：战场地图层+白刃战层+战术点系统**。**部曲、军屯/民屯、爵位加成、S21串联未实装** |
| S11 | 人事 | **M+** | 搜索/登用 + 任命三轨 |
...
| S20 | **前端体验** | **S/D** | Session 122 已实装己方武将名册、OfficerDetail、低忠诚警报，以及搜索/登用/任命/解职/赏赐统一终审窗；W4 其余子项与 W1~W3 仍为设计中，详见 `07-ui-design.md` §11.1.4/§12 |
...
*Session 122 交接 | 2026-07-20 | 武将名册/详情 + 人事操作终审窗已实装并经 Headless Chrome 实际操作验证；S20-W4 其余子项仍待续；0-B仍暂缓*
