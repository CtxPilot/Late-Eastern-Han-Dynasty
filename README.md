...
**EN** — The current build is a **0-A playable prototype**: two scenarios (Hero Assembly + a four-faction 190 CE slice), 24 scenario events, and the deterministic campaign/combat spine described above are playable today via the web client. Still intentionally absent: durable SQLite save/load, the full historical scenario set, delegated armies, private retinues, farming colonies, full formation progression (dual-axis growth / tech tree), the duel tournament, and the 0-B dataset expansion.

**中文** — 当前构建是 **0-A 可玩原型**：两个剧本（英雄集结 + 190《关东义兵》四势力切片）、24 个场景事件与上述确定性战役/战斗主线已在 Web 客户端可玩。尚未实装：SQLite 持久化存档、全量历史剧本、委任军团、部曲私兵、军屯/民屯、完整阵型成长（双轴/科技树）、单挑大会，以及 0-B 数据扩容。
